...
sub EVENT_SAY
{
 if($text =~ /hail/i)
  {
   quest::say("Greetings outsider. I trust you have [" . quest::saylink("I have peaceful intentions",0,"peaceful intentions") . "] while visiting our village?");
 }
 if($text =~ /peaceful intentions/i) {
   ...
   quest::say("I would ask you for [" . quest::saylink("I will help",0,"help") . "] but you are a stranger and owe us nothing. Of course if you did assist me, I would be forever greatful.");
 }
 if($text =~ /help/i) {
   quest::say("Oh thank you! You are most kind. I fear that there is not much time left for my dear Yuio. Please speak to [" . quest::saylink("Who is Khonza Ayssla?",0,"Khonza Ayssla") . "]. She is most wise and perhaps can find some cure for this affliction.");
 }
