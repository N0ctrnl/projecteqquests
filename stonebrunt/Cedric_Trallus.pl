...
sub EVENT_SAY {
  if($text=~/hail/i) {
    quest::say("Hello $name, chancing a journey to Brokenskull are you? Many have ventured there, but few have returned. I was favored by the will of Thule to return here in one piece, although I would not call my [" . quest::saylink("What journey?",0,"journey") . "] exactly successful.");
  }
  if($text=~/journey/i) {
    quest::say("Our spies in Erudin were able to learn that a voyage of Deepwater knights was setting sail in search of Brokenskull Island. Our priests in Paineel had received several visions from the lord Cazic telling of great powers that might be found on this island. Our group was instructed to follow their ship across the waters and explore the island should we find it. Find it they did, and more then they ever bargained for on that [" . quest::saylink("What cursed isle?",0,"cursed isle") . "].");
  }
  if($text=~/cursed isle/i) {
    quest::say("Just as the Deepwater knights were about to drop anchor near a small cove on the island, we attacked them. The magic cloaks we had placed over our ship did their job all too well. We took them by surprise and our forces easily overpowered them, but our battle was joined by a [" . quest::saylink("What third force?",0,"third force") . "].");
  }
  if($text=~/third force/i) {
    quest::say("A group of strange creatures crawled up the side of the boats and began to attack everyone on both sides. They cut through many of us before we were able to retreat our forces back to our ship and turn our sails away from the island. Several of the creatures pursued us through the water with amazing speed. We fought them off and we forced them back into the sea. We were able to slay one of them, or so we thought. We brought the [" . quest::saylink("What body?",0,"body") . "] back to Paineel for the priests to study.");
  }
  if($text=~/body/i) {
    quest::say("When we docked here in Paineel we were carrying the body off of the ship when suddenly the creature came to life. It threw several of our knights into the water and then jumped in himself and swam off into the ocean. We suspect that it used some form of magic to fool us into believing it was actually dead. The necromancers of Paineel are not easily fooled in matters of life and death. We greatly desire to study the [" . quest::saylink("What organs?",0,"organs") . "] of these creatures to learn the secret of this new magic.");
  }
  if($text=~/organs/i) {
...
sub EVENT_ITEM {
  if(plugin::check_handin(\%itemcount, 59035 => 2)) { #Luggald Organs
    ...
    quest::me("As you take the scroll from him you notice a sickly burning odor in the air. You glance down at Cedric's robes and notice several holes beginning to form where the blood touches his [" . quest::saylink("What robes?",0,"robes") . "].");
    quest::exp(300);
  }
