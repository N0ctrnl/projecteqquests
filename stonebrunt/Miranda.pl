...
sub EVENT_SAY {
 if($text =~ /hail/i){
  quest::say("I'm not supposed to talk to strangers but if my parents let you in here I guess it's ok. Do you like [" . quest::saylink("Who doesn't like candy?",0,"candy") . "]? I could give you some [" . quest::saylink("Give me some candy",0,"candy") . "] to play a game with me but my [" . quest::saylink("Where are your dice?",0,"dice") . "] are gone now.");
 }
 if($text =~ /candy/i){
