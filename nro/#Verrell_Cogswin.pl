...
sub EVENT_SAY {
  ...
  if(defined($qglobals{Green_Scale_Quest}) && ($qglobals{Green_Scale_Quest} == 2)) {
    ...
    if($text=~/bad news/i) {
      quest::say("The scale is no longer red it is gray. I confused the color change it is red to gray and gray to green, no need to fret tough I can just apply more and change it to green but there is a [" . quest::saylink("What is the catch?",0,"catch") . "].");
    }
    if($text=~/catch/i) {
      quest::say("The scale split into four pieces and the only way I can fix it is by applying a special [" . quest::saylink("What special adhesive?",0,"adhesive") . "].");
    }
    if($text=~/adhesive/i) {
      quest::say("In order to make this special adhesive you will need to combine some [" . quest::saylink("What glue powder?",0,"glue powder") . "] and some [" . quest::saylink("What is Deep Barley Brew?",0,"Deep Barley Brew") . "] in this container with this special vial. Return the result to me and I should be able to fix the scale, but please try to hurry I have to meet my brother Pellillo Cogswin at the wizard platform so he can take me home.");
...
sub EVENT_ITEM {
  if(plugin::check_handin(\%itemcount, 13006 => 1)) { #Water Flask
    quest::say("Thank Brell your kindness will not go unnoticed. I am eager to get back home I have been in the cold environment of Halas for so long, I thought I was going to turn into a snowgnome. I am just glad we finally figured out the [" . quest::saylink("What is the recipe?",0,"recipe") . "] for that concoction.");
    ...
    if(defined($qglobals{Green_Scale_Quest}) && ($qglobals{Green_Scale_Quest} == 1)) {
      ...
      quest::say("Hmmm . . well I have some [" . quest::saylink("What is the good news?",0,"good news") . "] and a lot of [" . quest::saylink("What is the bad news?",0,"bad news") . "] which do you want to hear first?");
