...
sub EVENT_SAY{
  if($text=~/Hail/i) {
    quest::say("Well met $name. What exactly is it I can do for you? You must excuse me if there are no pressing matters, I have a very important [" . quest::saylink("Who is the important lieutenant?",0,"lieutenant") . "] to track down. He has been missing for quite some time now.");
  }
  if($text=~/lieutenant/i) {
    quest::say("Ah yes of course the one I speak of is none other than Gerod Yalronsan, he is one of the finest warriors to ever serve the brotherhood. He was recently promoted to official courier of the brotherhood. He has most recently been tasked with the responsibility to take documents to and from Mistmoore. Most notably our battle strategies that are very dangerous if they end up in the wrong hands. I do not know what has become of him and I have grown worried. Would you be [" . quest::saylink("I am willing to help you locate Gerod",0,"willing to help me locate Gerod") . "].");
  }
