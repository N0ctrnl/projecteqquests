...
sub EVENT_SAY{
  if($text=~/Hail/i) {
    quest::say("What have we here? A new weakling that wishes to reap some sort of self-value and renown through service to the Indigo Brotherhood perhaps? Well speak up whelp!! Have you come to [" . quest::saylink("I will destroy the enemies of Neriak",0,"destroy the enemies of Neriak") . "] or are you a coward better suited to mop the filth from the back rooms of the Maidens Fancy.");
  }
  if($text=~/destroy the enemies of Neriak/i) {
    if (($class eq "Warrior") && ($race eq "Dark Elf")){
      quest::say("If you are going to be of any value to the Indigo Brotherhood you will need to outfit yourself in a suit of armor that will turn aside the blades of enemy forces. From the looks of you even the tiny fangs of a moss snake could bring you death. Take this request form to Issva H'Rugla to get yourself started on the assembly of your armor. When you have procured some protection for your weakling body return to me for [" . quest::saylink("What further instruction?",0,"further instruction") . "].");
  ...
  if($text=~/further instruction/i) {
    quest::say("Ah, you are eager to advance further within the Indigo Brotherhood. Although it is my duty to aid your training, do not allow your arrogance to blind you to your lowly position amongst the Kings most loyal subjects. Your next task is to assist the construction of a [" . quest::saylink("What weapon?",0,"weapon") . "] and [" . quest::saylink("What shield?",0,"shield") . "] worthy of being wielded by a Warrior of the Indigo Brotherhood.");
  }
  if($text=~/weapon/i) {
