...
sub EVENT_SAY { 
  ...
  if($text=~/materials/i){
    quest::say("Many species of dangerous creatures, the walking dead, and invading Deathfist Orcs inhabit the Nektulos Forest. You must defeat these creatures and undead in order to obtain the materials. The materials you seek depend on the piece of armor you desire. Do you desire to craft [" . quest::saylink("gauntlets") . "], [" . quest::saylink("boots") . "], a [" . quest::saylink("bracer") . "], a [" . quest::saylink("helm") . "], [" . quest::saylink("greaves") . "], [" . quest::saylink("vambraces") . "], or a [" . quest::saylink("breastplate") . "]?");
  }
  if($text=~/gauntlets/i) {
  ...
  if($text=~/boots/i) {
    quest::say("To craft Orc Slayer Boots you require two [" . quest::saylink("") . "], two cracked pyre beetle carapaces, and two orc foot bones. Once you have the necessary components combine them in your Mail Assembly Kit with this Tattered Boot Pattern.");
