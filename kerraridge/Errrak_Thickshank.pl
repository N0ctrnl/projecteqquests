...
sub EVENT_SAY {
  if($text=~/Hail/i) {
    quest::say("What?  What do you want. furless?  My tseq. Graalf. lies sick and you waste time with talk when you [" . quest::saylink("I can help",0,"cannot help") . "].");
  }
  if($text=~/help/i) {
    quest::say("The shamans. they have said I need [" . quest::saylink("What talismans?",0,"talismans") . "] to ward the sickness from Graalf.");
  }
  if($text=~/talisman/i) {
...
sub EVENT_ITEM {
  if (plugin::check_handin(\%itemcount, 7005 => 1, 13737 => 1, 13762 => 1)) {
    ...
    quest::summonitem(10139); # Item: Bone Talisman
  }
}
