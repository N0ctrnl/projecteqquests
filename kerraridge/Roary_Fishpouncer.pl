...
sub EVENT_SAY {
  if($text=~/Hail/i) {
    quest::say("Rrrr.. catching and prrreparing fish is my specialty. Perrrhaps you could fetch for me some [" . quest::saylink("What rrrare fish?",0,"rrrare fish") . "] so that I may demonstrate my skill? Rrrr.");
  }
...
sub EVENT_ITEM {
  if (plugin::check_handin(\%itemcount, 12318 => 1)) {
    ...
    quest::summonitem(7027); # Item: Kerran Fishing Spear
  }
}
