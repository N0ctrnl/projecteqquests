...
sub EVENT_SAY { 
  if($text=~/Hail/i) {
    quest::say("Rarr.  Me is Feren.  Me is fisher.  That means Feren catches [" . quest::saylink("What fish?",0,"fish") . "].");
  }
  if($text=~/fish/i) {
    quest::say("All kinds of fish.  Tasty fish.  All day Feren catches the fishes so all Kerran can eat.  I do for good of all Kerran.  Rrrrr.  One thing bother Feren always though.  Is bad fish.  [" . quest::saylink("Who is Razortooth?",0,"Razortooth") . "].");
  }
