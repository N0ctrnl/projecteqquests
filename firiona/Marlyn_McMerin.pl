...
sub EVENT_SAY {
  if($text =~ /hail/i) {
    quest::say("Greetings hearty adventurer. Searching for the components necessary for fine shaman spells are we? I have come to Kunark in search of [" . quest::saylink("rare alchemy components") . "], but I have found the dangers of Kunark are far too great for me.");
  }
  if($text=~/rare alchemy components/i){
    quest::say("Grand!! I have heard word of [" . quest::saylink("four rare components") . "]. Collect them for me and I will share with you a rare scroll of [" . quest::saylink("McMerin's Feast") . "].");
  }
  ...
  if($text=~/four rare components/i){
    quest::say("From the lands of Kunark we require some clay of Ghiosk, crushed dread diamond and some powder of a Yun shaman. The rarest component is the bones of one who touched the Bath of Obulus. Find these things, return them to me and the secret of [" . quest::saylink("McMerin's Feast") . "] is yours.");
  }
}
