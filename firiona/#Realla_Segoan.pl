...
sub EVENT_SAY {
  if ($text=~/hail/i) {
    quest::say("Greetings, $name. I am Realla Segoan from the city of Erudin. I cannot spend time chatting with you now. I have [" . quest::saylink("official tasks") . "] to perform.");
  }
  if ($text=~/official tasks/i) {
    quest::say("I have been sent to this continent by the High Council of Erudin to find information about the disappearance of [" . quest::saylink("Al'Kabor") . "].");
  }
  if ($text=~/al'kabor/i) {
    quest::say("Al'Kabor is believed to be the most powerful living wizard on Norrath. In addition, he is also one of Erudin's oldest citizens. The High Council is very interested in receiving information about his disappearance. So far, from the [" . quest::saylink("reports") . "] that I have collected, there appears to have been a [" . quest::saylink("troublemaking gnome") . "] involved.");
  }
  if ($text=~/reports/i) {
    quest::say("I have received many reports regarding the disappearance. Adventurers upon this continent have reported sightings of some of Al'Kabor's [" . quest::saylink("missing resarch notes") . "].");
  }
  ...
  if ($text=~/missing research notes/i) {
    quest::say("It appears that through his travels, the great wizard has misplaced some of his research notes. Perhaps if I can find someone to [" . quest::saylink("help collect the research notes") . "], the High Council may be able to determine what caused Al'Kabor's disappearance. However, only those who truly know the path the great wizard's research has taken him will know where to find the missing research notes.");
  }
