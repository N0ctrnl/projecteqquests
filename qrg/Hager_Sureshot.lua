...
function event_say(e)
	if(e.message:findi("hail")) then
		e.self:Say("Greetings, "..e.other:GetName()..".  Feel free to rest here in Surefall Glade.  Just be sure you do no hunting whilst here.  If so, then you shall pay the punishment of death.  I trust you [" .. eq.say_link("I will abide by the laws",false,"will abide by the laws") .. "]?");
	elseif(e.message:findi("abide by the laws")) then
		e.self:Say("That is wise.  Your health, for now, is assured.  A plague of hunters has infested our land heavily in recent weeks.  The word is out that [" .. eq.say_link("What Mammoth?",false,"Mammoth") .. "] still lives.  Hunters have flocked here in search of a trophy or profit and glory.  Instead, they find the deadly accuracy of my arrows.");
	elseif(e.message:findi("mammoth")) then
		e.self:Say("Mammoth is the great bear of the caves.  [" .. eq.say_link("Who is Tunare?",false,"Tune") .. "] has blessed our home with Mammoth's presence.  It is said that when the great bear dies, so too shall the Jaggedpine.  He would be quite a trophy to some hunter.  And quite profitable to a poacher. Even the infamous [" .. eq.say_link("Who is Talym Shoontar",false,"Talym Shoontar") .. "] has tried his hand at the hunt.");
	...
	elseif(e.message:findi("talym shoontar")) then
		e.self:Say("Talym Shoontar is a poacher.  He has eluded us for quite some time.  He has organized parties of poachers to plague our forest as well as others.  He hails from Halas, but I have heard he now calls the Plains of Karana home.  I have placed a bounty upon his head.  Do you [" .. eq.say_link("I wish to collect the bounty",false,"wish to collect the bounty") .. "]?");
...
function event_trade(e)
	...
	if(item_lib.check_turn_in(e.trade, {item1 = 18709})) then
		...
		e.other:Faction(306,-15,0); -- Sabertooths of Blackburrow
		e.other:Faction(262,25,0); -- Guards of Qeynos
		e.other:AddEXP(100);
	elseif(item_lib.check_turn_in(e.trade, {item1 = 13913})) then
		...
		e.other:Faction(306,-30,0); -- Sabertooths of Blackburrow
