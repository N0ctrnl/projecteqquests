...
function event_say(e)
	if(e.message:findi("hail")) then
		e.self:Say("Welcome, friend! I hope your stay in Surefall Glade will ease your mind and soothe your soul. While you are here, we ask you to abide by the [" .. eq.say_link("What laws of the Jaggedpine?",false,"laws of the Jaggedpine") .. "].");
	elseif(e.message:findi("laws of the jaggedpine")) then
		e.self:Say("We ask that you do not start campfires on our land. Do not litter our land with any items. Do not chop down our pines or tread upon any foliage. Please do not take more than a bellyful of fish. Do not harm any other wildlife and stay clear of [" .. eq.say_link("What is the great bear?",false,"the great bear") .. "]!");
	elseif(e.message:findi("great bear")) then
		e.self:Say("The great bear I speak of is Mammoth. He has lived among us for over a century. Tunare has truly blessed us with his presence. As long as Mammoth lives, so too does Surefall Glade. We are currently worried. Corun has reported that there is something [" .. eq.say_link("What is wrong with Mammoth?",false,"wrong with Mammoth") .. "].");
	elseif(e.message:findi("wrong with mammoth")) then
		e.self:Say("Mammoth is not eating. We believe something is lodged in his throat. We require a young druid to [" .. eq.say_link("I will fetch a special potion",false,"fetch a special potion") .. "].");
