...
function event_say(e)
	if(e.message:findi("hail")) then
		e.self:Say("Welcome, friend. Welcome to Surefall Glade. I need not remind you about [" .. eq.say_link("What about poaching?",false,"poaching") .. "], I hope");
	elseif(e.message:findi("poaching")) then
		e.self:Say("Poaching is illegal. We here are the Protectors of the Pine, rangers sworn to protect our land and all its inhabitants. This includes the wildlife. We spend a lot of time hunting poachers. If you want to [" .. eq.say_link("I want to join the hunt",false,"join the hunt") .. "], just let me know.");
...
function event_trade(e)
	...
	elseif(item_lib.check_turn_in(e.trade, {item1 = 13825})) then
		e.self:Say("Your deeds are great indeed. We shall cleanse our land of these poachers once and for all. A report has surfaced of a [" .. eq.say_link("What master poacher?",false,"master poacher") .. "].");
		e.other:Ding();
		e.other:Faction(302,3,0); -- Protectors of Pine
		e.other:Faction(272,3,0); -- Jaggedpine Treefolk
		e.other:Faction(306,-3,0); -- Sabertooths of Blackburrow
