...
function event_say(e)
	...
	elseif(e.message:findi("interested")) then
		e.self:Say("I have messages that need to go to - well, right now I have one that needs to go to Freeport.  Will you [" .. eq.say_link("I will deliver to Freeport",false,"deliver mail to Freeport") .. "] for me?");
	...
	elseif(e.message:findi("deliver to freeport")) then
		...
		e.other:SummonItem(18157); -- Item: Bardic Letter (Freeport)
	end
end
