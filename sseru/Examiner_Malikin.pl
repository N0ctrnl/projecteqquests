...
sub EVENT_SAY {
  ...
  if($text=~/simple task/i){
    quest::say("Simple tasks go a great distance to establish trust. I may know of someone who can help you establish yourself within the ranks of the Eye. If you are interested in performing a task of that nature I can get you started at this time. Would you like to [" . quest::saylink("I would like to begin",0,"begin") . "]?");
  }
  if($text=~/begin/i){
  ...
  if($text=~/meaning/i){
    quest::say("Very good, for I have another task to offer you if you would like to continue. This one is a bit more complicated. Im sure that you can handle it, but I need you to be sure as well. I need this to be a suprise. Do you think that you are [" . quest::saylink("I am prepared",0,"prepared") . "] to handle an issue that will require both stealth and secracy?");
  }
  if($text=~/prepared/i){
  ...
  if($text=~/attacks/i){
    quest::say("We have found evidence that there is a traitor within the ranks of one of the other Praesurtum. We need you to help us remove this traitor and his contact in Katta Castellum. You will need to take this poison and pour it into our [" . quest::saylink("Who is our target?",0,"target's") . "] ear while he sleeps.");
