...
function event_say(e)
	if(e.message:findi("hail")) then
		e.self:Say("Good day to ye! Ye look a wee bit hungrry. I sell many fine foods. Please have a look. Might I suggest the [" .. eq.say_link("What lion roast?",false,"lion roast") .. "]? It is a wee bit expensive, but well worth it.");
	end
	if(e.message:findi("lion roast")) then
		e.self:Say("The lion roast is quite good. Tis me specialty. Ah marinate the lion meat in mammoth fat fer two days before roasting it. Lion meat isn't common around here. I'll purchase what I can from strangers but I rely mostly on shipments from the Plains o' Karana. I'm presently searching fer someone to [" .. eq.say_link("I will pick up the lion meat shipment",false,"pick up me lion meat shipment") .. "].");
	end
	...
	if(e.message:findi("lion delight")) then
		e.self:Say("Oh, yes. A wonderful tasting dish. Unfortunately, I need more lion meat to make that dish. Might I instead suggest the [" .. eq.say_link("What lion roast?",false,"Lion roast") .. "]?"); --Text made up
	end
end
...
function event_trade(e)
	...
	if(item_lib.check_turn_in(e.trade, {item1 = 13961})) then 	--Lion Meat Shipment
		e.self:Say("Ye've returned!! How wonderful! The people o' Halas thank ye! It isn't often we get to indulge ourselves in the delicacies o' warmer climates. Here ye go, me friend. Ye've completed the delivery in good time. I hope ye deliver more often. Here, try some of me new creation.. [" .. eq.say_link("What Lion Delight?",false,"Lion Delight") .. "].");
