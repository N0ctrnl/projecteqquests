...
sub EVENT_SAY{   
  if ($text=~ /Hail/i){
    quest::say("Greetings and welcome. I have been compiling reports from all over Norrath! There are many odd jobs to be done by someone like you. Would you be interested in such a [" . quest::saylink("What quest?",0,"quest") . "]?");  
  }
  if ($text=~ /quest/i){
    my @task_array;
    push(@task_array, 130);
