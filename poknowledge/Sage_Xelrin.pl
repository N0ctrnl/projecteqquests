...
sub EVENT_SAY { 
  if($text=~/Hail/i){
    quest::say("Greetings $name.  I assume you are here to ask about my research?  I imagine that word has spread of my groundbreaking approach to spell creation.  Well I am always willing to enlighten those that choose to learn.  As not to bog you down with too many details. my theory is simple. [" . quest::saylink("continue1",0,"continue") . "]");
  }
  if($text=~/continue1/i){
    quest::say("Well the idea is rather basic.  As I am sure you know. there are many magical artifacts in the world with effects that we have not mastered.  It is my conjecture that some of the magic may be extracted from these items in order to further our magical repertoire.  Even in the case where an object has a spell that we already have access to. I believe the binding used to tie the magic to the object could be useful as well. [" . quest::saylink("continue2",0,"continue") . "]");
  }
  if($text=~/continue2/i){
    quest::say("By using this box created in cooperation with the [" . quest::saylink("What other sages?",0,"sages") . "] in Knowledge. you will be able to either extract a magical scroll or the binding used to hold the magic to the object.  Those elements will be the basis for my research.  Just bring anything you manage to extract for me.");
