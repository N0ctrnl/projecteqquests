...
sub EVENT_SAY {
  if($text=~/Hail/i && plugin::check_hasitem($client, 1057)){
    quest::say("Wow, you have a Mystic Cloak! I was just reading a book about cloaks with similar enchantments. I'm studying to try and learn how to make magical and enchanted armor. I'm pretty sure I have figured out a way to [" . quest::saylink("How can you recharge?",0,"recharge") . "] a Mystic Cloak.");
  }
  elsif($text=~/Hail/i) {
  	quest::say("Hello there.  Fine afternoon. isn't it?");  	
  }
  if($text=~/recharge/i) {
    quest::say("Well, in the case of the Mystic Cloak at least, the true enchantment is in the brooch that you use to clip the cloak around your shoulders. The cloak itself cannot run out of magic, the brooch, however, can. When it no longer has the ability to cast spells you should be able to attach a [" . quest::saylink("What new brooch?",0,"new brooch") . "] and regain the magic the cloak once possessed.");
  }
