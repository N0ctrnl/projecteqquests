...
##############################################################################################################################
##############################################################################################################################
  if ($text =~ /Tower of Solusek Ro/i
      && (   !defined $qglobals{pop_sol_ro_arlyxir}
          || !defined $qglobals{pop_sol_ro_dresolik}
  ...
  { if (defined $qglobals{pop_sol_ro_arlyxir}) {
    ...
    if (defined $qglobals{pop_sol_ro_solusk}) {
      $client->Message(12, "You have defeated Solusek Ro within his own tower.  Please make sure you've completed events in the [" . quest::saylink("plane of innovation", 0, "Plane of Innovation") . "] and [" . quest::saylink("fortress of zek", 0, "Drunder, Fortress of Zek") . "].");
    } else {
      $client->Message(9, "You have NOT defeated Solusek Ro within the Tower of Solusek Ro.  Please make sure youve completed events in the [" . quest::saylink("plane of innovation", 0, "Plane of Innovation") . "] and [" . quest::saylink("fortress of zek", 0, "Drunder, Fortress of Zek") . "].");
    }
  }
  if (   $text =~ /Tower of Solusek Ro/i
      && defined $qglobals{pop_sol_ro_arlyxir}
...
##############################################################################################################################
  ...
  if (   $text =~ /elemental flag/i
      ...
      && defined $qglobals{pop_pov_aerin_dar}
      && $qglobals{pop_pos_askr_the_lost} == 3
      && defined $qglobals{pop_pos_askr_the_lost_final}
      && defined $qglobals{pop_cod_preflag}
      && defined $qglobals{pop_cod_bertox}
      && defined $qglobals{pop_cod_final}
      && defined $qglobals{pop_pot_shadyglade}
...
##############################################################################################################################
  ...
  if (   $text =~ /I am ready to access the Temple of Marr/i
      && defined $qglobals{pop_pon_hedge_jezith}
      && defined $qglobals{pop_pon_construct}
      && defined $qglobals{pop_ponb_terris}
      && defined $qglobals{pop_ponb_poxbourne}
