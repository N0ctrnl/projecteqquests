...
sub EVENT_SAY {
  if($text=~/Hail/i) {
    quest::say("A fine day to you, $name. The exhibit you will see here soon will likely be the most valuable of them all. My display will house some of the most precious and rare gemstones you will ever find. It is too sad there are still spaces missing in my display box, if I could only find a few more [" . quest::saylink("What gems?",0,"gems") . "] to fill them out.");
  }
  if($text=~/gems/i) {
    quest::say("To make the exhibit truly stand out, I would need to add a Blackened Sapphire, Greenscale Emerald, Shimmering Velium Ruby, and a Hope Diamond. Could I perhaps procure these from you? Visit [" . quest::saylink("Who is Merri?",0,"Merri") . "] and get a Collector's Box from her. Carefully place the gems inside and close it up. Once you return this to me, I can pay you for your services.");
  }
  if($text=~/merri/i) {
