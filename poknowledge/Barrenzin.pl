sub EVENT_SAY {
  if($text=~/hail/i) {
    if(quest::istaskactivityactive(207,4)) { #Ambushed! Step 3
      ...
      quest::say("'Greetings, $name,' Barrenzin smiles as he hands you a glass of wine. 'Thank you for coming on such short notice. Please make yourself comfortable and then we can discuss [" . quest::saylink("What business?",0,"business") . "].'");
    }
  }
  if($text=~/business/i) {
    quest::say("'A $class that gets right to point,' Barrenzin smiles again. 'Such a refreshing change of pace from the pandering rabble I normally deal with. Allow me to be blunt then. I recently lost several, let's say, 'associates' of mine and I'm a little understaffed at the moment. I've been following your adventuring career since Gloomingdeep and I think you could go far in my [" . quest::saylink("What organization?",0,"organization") . "]. What say you? [" . quest::saylink("I am interested",0,"Interested") . "] in making a name for yourself? Or perhaps you have a [group] assembled that's looking for a little coin?'");
  }
