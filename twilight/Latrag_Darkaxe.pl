...
sub EVENT_SAY { 
  if($text=~/Hail/i){
    quest::say("Har there lad! Ye be sure 'nuff to tell em all that Brewmaster Latrag is [" . quest::saylink("He is retiring?",0,"retiring") . "].");
  }
  if($text=~/retiring/i){
    quest::say("Ya got that right. I'm goin' to concentrate more on me devotion to Brell as a [" . quest::saylink("What is a holy knight?",0,"holyl knight") . "]!");
}
  if($text=~/holy knoght/i){
    quest::say("I be a paladin of Brell thar Daias . Might ye be a [" . quest::saylink("I am a paladin",0,"paladin") . "] yerself?");
}
  if($text=~/paladin/i){
    quest::say("That's good. Ya know I have some armor ye might be interested in.");
}
  if($text=~/what armor/i){
    quest::say("Yar! I have me a [" . quest::saylink("helm",0,"helm") . "], [" . quest::saylink("breastplate",0,"breastplate") . "], [" . quest::saylink("vambraces",0,"vambraces") . "], [" . quest::saylink("greaves",0,"greaves") . "], [" . quest::saylink("pauldrons",0,"pauldrons") . "], and a [" . quest::saylink("bracer",0,"bracer") . "]. If ye want the rest talk to the mighty holy warrior Trallen over there about armor.");
}
  ...
  if($text=~/what pauldrons/i){
    quest::say("For the pauldrons ye gotta bring me a meteor jewel, a mark of daring, and a l  ife gem.");
}
...
sub EVENT_ITEM { 
  ...
  } elsif (plugin::check_handin(\%itemcount, 4494 => 1,5900 => 1,5901 => 1)){ # meteor jewel, a mark of daring, and a l  ife gem
