...
sub EVENT_SAY { 
  ...
  if($text=~/i am a paladin/i){
    quest::say("Of course ya are $name. why would have ya asked me about the armor if ye wasn't eh. I have the [" . quest::saylink("boots",0,"boots") . "], [" . quest::saylink("mask",0,"mask") . "], [" . quest::saylink("cloak",0,"cloak") . "], [" . quest::saylink("gauntlets",0,"gauntlets") . "], [" . quest::saylink("gorget",0,"gorget") . "], [" . quest::saylink("girdle",0,"girdle") . "], and a [" . quest::saylink("sword",0,"sword") . "]. Which do ye want?");
  }
  if($text=~/what mask/i){
