...
sub EVENT_SAY {
  if($text=~/hail/i) {
    quest::say("Ahh...yes, I've read reports about you. Always showing up here and there across Luclin. Not always under the best circumstances though, eh? No matter. I need someone who can get [" . quest::saylink("I can get things done",0,"get things done") . "]. I'm not looking for a saint.");
  }
  if($text=~/things done/i) {
    quest::say("I've been collecting information on the Grimlings and a disturbing pattern is emerging. Although the military-minded Vah Shir are focused on the Grimling battle strength, I fear there is something else growing in the [" . quest::saylink("What dark depths?",0,"dark depths") . "] of those caverns.");
  }
  if($text=~/dark depths/i) {
    quest::say("There is a legend of a Vah Shir name Khati Sha. He was one of my people's first great heroes. An explorer and leader, he mysteriously disappeared long ago. I have the unsettling feeling that this war will uncover the [" . quest::saylink("What secret?",0,"secret") . "] of his disappearance.");
  }
  if($text=~/secret/i) {
...
sub EVENT_ITEM {
  if(plugin::check_handin(\%itemcount, 63163 => 1)) { #Legend of Lies
    quest::say("This is disturbing. Is there no corner of place or time that the Shissar have not tainted? I must know more. The Vah Shir people must know the truth of their [" . quest::saylink("hero") . "].");
