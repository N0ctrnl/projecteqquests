...
function event_say(e)
	...
	elseif(e.message:findi("request")) then
		e.self:Say("You have no request for me? How odd. Well, then. Perhaps a tale of [" .. eq.say_link("What history?",false,"history") .. "] will do?");
	elseif(e.message:findi("history")) then
		e.self:Emote("strums his lute and clears his throat. 'Here is a tale of the place of torment from which souls good and bad have come to call home upon this earthly [" .. eq.say_link("What realm?",false,"realm") .. "].'");
	elseif(e.message:findi("realm")) then
		e.self:Say("The place I speak of is the Estate of Unrest. It is a place that the dead roam and the living detest. Garanel Rucksif was his name, a name that no longer came to be . A name that Quellious herself saw fit not to let anyone see, for a cleric loyal to her called the lands home. Open to all, the Fier'Dal, Koada'Dal and even the occasional [" .. eq.say_link("What gnome?",false,"gnome") .. "].");
	elseif(e.message:findi("gnome")) then
		e.self:Say("A mansion built by the cleric with care, for he came to spread the word of Quellious to Faydwer. Garanel mad with rage, killed his wife and children three. From his dwarven homelands he now will flee. Upon this place of tranquility he stumbled upon. He tortured and killed the cleric and his kin by morning's dawn. Within the sacred chapel walls the bloodshed took place. Eyes wild with rage, madness and blood upon his [" .. eq.say_link("What about his face?",false,"face") .. "].");
	elseif(e.message:findi("face")) then
		e.self:Say("Quellious, enraged by this act of violence and murder, decreed that this would continue no further! Garanel was struck down, his soul cursed forever more, to haunt the grounds of the defiled estate, guarding its bloody floor. Undead monsters, ghosts and [" .. eq.say_link("What ghouls?",false,"ghouls") .. "] slaughter all those who stray. They say his undead power grows within him to this day. This sad tale, my friends, is the Legend of Unrest.");
	elseif(e.message:findi("ghouls")) then
		e.self:Say("I've even heard of the ghosts of poor children haunting the estate. Have you ever [" .. eq.say_link("I have seen a child's ghost",false,"seen a child's chost") .. "]?");
	elseif(e.message:findi("seen a child's ghost")) then
		e.self:Say("You have? Did it speak to you?  What did it say?  Only the essences of those unjustly wronged or the vilest of souls are bound to this realm and sometimes end up in that accursed place.  Only that which binds them to this realm may set them free.   Curse those who would harm an innocent child!  I knew of one such unfortunate child.  I was to bring her a [" .. eq.say_link("What doll?",false,"doll") .. "] for her birthday but she fell to a thieving cutthroat's sword!'  Baenar's eyes fill with tears.  'Pardon me, friends, she was like a daughter to me.  I loved her as I would my own!");
...
function event_trade(e)
	...
	if(item_lib.check_turn_in(e.trade, {item1 = 20373})) then
		e.self:Say("So, Belious Naliedin sent you, eh?' Baenar shakes his head. 'I hope your letter of introduction did not empty your purse. You have the audience you seek. What is your [" .. eq.say_link("What request?",false,"request") .. "]? A song perhaps? To immortalize your heroic deeds in song? How about an epic battle with a gnoll pup? I'll make you famous.");
