...
sub EVENT_ITEM {
 if (plugin::check_handin(\%itemcount, 67054 => 1)) {
	quest::say("Oh thank Brell I thought I would never be found. My brother Pellillo is right we were wrong to take the paper and I realized that shortly after, but when I started the journey back I was captured by a band of Sarnaks and brought here. When they found the paper, the Queen demanded that it be researched and so she gave it to [" . quest::saylink("Historian Selrin") . "].");
        quest::unique_spawn(103217,0,0,1291.27,-559.92,-159.75); # NPC: #Historian_Silren_Di`zok
 }
}
