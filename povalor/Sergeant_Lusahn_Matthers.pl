sub EVENT_SAY {
  if($text=~/hail/i) {
    quest::say("Greetings $name. I have a little time to chat, we are not due in the [" . quest::saylink("What field?",0,"field") . "] for a while. What may I help you with?");
  }
  if($text=~/field/i) {
    quest::say("The Glassy Wastelands. We work hard to keep the [" . quest::saylink("What plight?",0,"plight") . "] back from the rest of the Forest, but I fear that is a losing battle.");
  }
  if($text=~/plight/i) {
