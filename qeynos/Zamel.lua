...
function event_timer(e)
	...
	if(e.timer == "help") then
		...
		if(endicRandom < 16) then
			e.self:Say("Clean the [" .. eq.say_link("What aqueducts?",false,"aqueducts") .. "] they tell me. Peh, they don't pay me enough to climb into that filth!");
		end		
	end
end
...
function event_say(e)
	if(e.message:findi("hail")) then
		e.self:Say(string.format("Greetings, good %s.  My name is Zamel and I have the much coveted duty of pulling sludge up out of the pools and [" .. eq.say_link("aqueducts",false,"aqueducts") .. "] of Qeynos..",e.other:GetName()));
	elseif(e.message:findi("aqueducts")) then
		e.self:Say("Aye, the aqueducts are a horrible place.  I have heard rumors of horrible [" .. eq.say_link("What beasts?",false,"beasts") .. "] who live under the city in the water and just the other day, a child playing in the water just up and disappeared!");
	elseif(e.message:findi("beasts")) then
		e.self:Say("I know nothing of what or where they may be, just rumors, friend.  However, should you foolishly choose to enter the waters to brave the possibilities and return with some evidence of the [" .. eq.say_link("What child?",false,"child's") .. "] fate, I would be most appreciative.");
