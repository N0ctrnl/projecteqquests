...
function event_say(e)
	if(e.message:findi("hail")) then
		e.self:Say("Oh.. Hiya, I'm just out here fishing, since I can't find a job. I hope this [" .. eq.say_link("What bait?",false,"bait") .. "] I just bought catches me a big ol' fish.");
	...
	elseif(e.message:findi("tacklebox")) then
		e.self:Say("Oh. That mean [" .. eq.say_link("What dwarf?",false,"dwarf") .. "], Trumpy, just knocked my tacklebox into the water. Could you please get it for me? I can't swim.");	
	elseif(e.message:findi("dwarf")) then
		e.self:Say("His name is Trumpy. He is one of those [" .. eq.say_link("Who are the Irontoes?",false,"Irontoes") .. "] I think. I've seen him hanging out in the Fish's Ale. I don't know why he likes to torment me.");
...
function event_signal(e)
	if(e.signal == 1) then
		e.self:Say("Oh, hi, Beren. Not too good so far. That [" .. eq.say_link("What dwarf?",false,"dwarf") .. "] keeps bothering me, too.");
...
function event_trade(e)
	...
	if(item_lib.check_turn_in(e.trade, {item1 = 13702})) then
		...
		e.self:Say("Thank you so much!  If you want some free advice, steer clear of those [" .. eq.say_link("Who are the Irontoes?",false,"Irontoes") .. "]! They are nothing but trouble. Here, It's not much but I must thank you somehow.");
		e.other:Ding();
