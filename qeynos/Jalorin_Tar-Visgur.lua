...
function event_say(e)
	...
	elseif(e.message:findi("sent from erudin")) then
		e.self:Say("Well what is it you have been sent for? Go ahead I don't have all day I must get these [" .. eq.say_link("What incantations?",false,"incantations") .. "] to the council and I am not finished with them yet.");
	elseif(e.message:findi("incantations")) then
		e.self:Say("You certainly do not look like you could read the scrolls I have been working on so I am wondering why you are questioning what they are. I am waiting for some scrolls from my friend Elisi. The last time I saw her she gave me this [" .. eq.say_link("What parchment?",false,"parchment") .. "] but I am waiting for her to complete it, would you be able to [" .. eq.say_link("I will return it",false,"return") .. "] it to Brianna for me? It is quite clear that she trusts you so I suppose I should as well.");
	...
	elseif(e.message:findi("Elisi")) then
		e.self:Say("You know, Elisi was a bit like you she didn't know when to quit. You and her will soon share the same [" .. eq.say_link("What fate?",false,"fate") .. "] I should think.");
