...
function event_say(e)
	if(e.message:findi("hail")) then
		e.self:Say(string.format("Greetings, %s.  My name is Vicus Nonad. <cough>  I am the official tax collector for the fine city of Qeynos. <cough>  I serve the will of Antonius Bayle, our glorious leader.  <cough>  <cough>  Please excuse my [" .. eq.say_link("What cough?",false,"couch") .. "].  <cough>",e.other:GetName()));
	elseif(e.message:findi("cough")) then
		e.self:Say("Oh, <cough> I am sorry, but it seems I have fallen a bit ill.  I was caught out in the rain the other day and my chills have gotten the best of me. <cough>  If only someone would [" .. eq.say_link("I will help",false,"help") .. "] me with today's [collections]..  <cough>");
...
function event_trade(e)
	...
	elseif(item_lib.check_turn_in(e.trade, {item1 = 13181})) then
		e.self:Say("Very good <cough> work. But I need both the full tax collection box and the list of debtors. You did get the [" .. eq.say_link("What list?",false,"list") .. "] from me before you left, right? <cough>");
