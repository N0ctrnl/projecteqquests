...
function event_say(e)
	...
	if(e.message:findi("hail")) then
		e.self:Say(string.format("Welcome, %s. I am Deadet Losaren, faithful priest of Karana. You appear to be in search of [" .. eq.say_link("I am in search of guidance",false,"guidance") .. "] or perhaps you are merely a [" .. eq.say_link("I am a visitor to the temple of Thunder",false,"visitor of the temple of Thunder") .. "] wishing the blessing of [" .. eq.say_link("I wish the blessing of holy armor",false,"holy armor") .. "]?", e.other:GetName()));
	elseif(e.message:findi("guidance")) then
		e.self:Say("So it is guidance you seek. You will begin on the bottom rung. There is much more to our temple than the bashing of foes. We will need you to [" .. eq.say_link("I will collect donations",false,"connect donations") .. "].");
	...
	elseif(e.message:findi("holy armor")) then
		e.self:Say("Holy Armor? I offer the blessing of Holy Armor to members in good standing of the Knights of Thunder. I must charge ten gold for the service. [" .. eq.say_link("What components?",false,"Components") .. "] are expensive these days!");
	elseif(e.message:findi("components")) then
		e.self:Say("Components are required for my daily tribute to the Rainkeeper. It is he whose power flows from my body. I require a faithful acolyte to [" .. eq.say_link("I will gather the required items",false,"gather the required items") .. "].");
	elseif(e.message:findi("gather the required items")) then
		if(fac < 5) then
			e.self:Say("Very noble of you. Take this chest. I will require it to be filled with flame beetle eyes. Four normal beetles and four [" .. eq.say_link("What Kerra Isle beetles?",false,"Kerra Isle beetles") .. "]. Combine these items within the chest and return it to me. I shall reward you with the protection of Holy Armor.");
