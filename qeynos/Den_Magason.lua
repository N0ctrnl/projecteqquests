...
function event_waypoint_arrive(e)
	if(e.wp == 16 or e.wp == 43 or e.wp == 70) then
		e.self:Say("Psst.. Hey, you there.. Yeah, you.. You need some [" .. eq.say_link("What blank scroll sheets?",false,"blank scroll sheets") .. "].. I just got a special shipment of things in.. Come on, it's just what you're looking for.. I know you.");
	end
end

function event_say(e)
	if(e.message:findi("hail")) then
		e.self:Say("Oh, hey there.. How ya doing? My name's Den. I work for ol' [" .. eq.say_link("Who is Rohand?",false,"Rohand") .. "] here, unloading the [" .. eq.say_link("What shipments?",false,"shipments") .. "] that come in from overseas.");
	elseif(e.message:findi("rohand")) then
		e.self:Say("Ol' Captain Rohand is the owner of the Mermaid's Lure. He trades goods with sailors and sea [" .. eq.say_link("Which merchants?",false,"merchants") .. "]. I think that old man was even born at sea.. And he's got more stories than fish have scales.");	
	elseif(e.message:findi("shipments") or e.message:findi("merchants")) then
		e.self:Say("Yeah.. We get merchants and traders with goods from here to [" .. eq.say_link("What about Odus?",false,"Odus") .. "].");	
