...
function event_say(e)
	...
	if(e.message:findi("hail")) then
		e.self:Say(string.format("Hail, %s.  I'm Kazlo Naedra of the Order of Three.  We always seem to be running out of [" .. eq.say_link("What supplies?",false,"supplies") .. "] around here, which is, of course, very distracting to our studies.",e.other:GetName()));
	...
	elseif(e.message:findi("supplies")) then
		if(fac < 5) then
			e.self:Say("Yes, we're almost out of [" .. eq.say_link("What blank scroll sheets?",false,"blank scroll sheets") .. "]. Gahlith will be very upset, unless I can replenish our inventory soon.");
		else
