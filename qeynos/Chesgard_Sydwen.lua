...
function event_say(e)
	...
	if(e.message:findi("hail")) then
		e.self:Say('May Karana guide your strengths.  Are you a [" .. eq.say_link("I am a citizen of Qeynos",false,"citizen of Qeynos") .. "] or a visitor?');
	-- Same dialogue for both : 'I am a citizen' and 'I am not a citizen'
	elseif(e.message:findi("citizen of qeynos")) then
		e.self:Say("Then I would urge you to attend daily services in the Temple of Thunder.  You are a [" .. eq.say_link("I am a member of Thunder",false,"member of Thunder") .. "] are you not?  Or maybe I am mistaken and you are just [" .. eq.say_link("I am lost",false,"lost") .. "].");
	elseif(e.message:findi("lost")) then
		e.self:Say("Then study and spread the words of Karana the Rainkeeper.  May all the storms in your heart be controlled by the Rainkeeper.  You are young to the world just as [" .. eq.say_link("Who is Cheslin?",false,"Cheslin") .. "] is.");
	elseif(e.message:findi("member of thunder") and fac <= 4) then
		e.self:Say("I welcome you. Karana cares for all of His flock. He bestows a touch of His power upon the souls of the Clerics and Paladins of our temple. We look after His flock. Speaking of which, I require the assistance of a young paladin to [" .. eq.say_link("I will deliver provisions to needy members",false,"deliver provisions to needy members") .. "].");
...
function event_trade(e)
	...
	if(item_lib.check_turn_in(e.trade, {item1 = 12100})) then
		e.self:Say("Good work $class. The Karana Bandits have been plaguing the Rainkeepers flock for some time.  Take this as a small reward for your devotion.  Continue the fight against the Karana Bandits.  Ahh... I wish [" .. eq.say_link("Who is Cheslin?",false,"Cheslin") .. "] was equally as skilled.");
