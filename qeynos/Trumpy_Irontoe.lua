...
function event_say(e)
	...
	if(e.message:findi("hail")) then
		e.self:Say(string.format("Heh? What's yer name? %s? Never heard of ya. Now quit breathin' on me and buy us some [" .. eq.say_link("What drinks?",false,"drinks") .. "]!",e.other:GetName()));
	...
	elseif(fac < 4) then
		if(e.message:findi("skippy")) then
			e.self:Say("Skippy? You mean that fish farmer? Heh heh heh. What a moron. And his brother [" .. eq.say_link("Who is Beren?",false,"Beren") .. "] has the nerve to actually threaten ME!?.. Burp.. Bah. Remind me to tell [" .. eq.say_link("Who is Kane?",false,"Kane") .. "] about that steaming pile of gnoll dung.");
		elseif(e.message:findi("beren")) then
			e.self:Say("Don't even mention that weakling's name. He.. burp.. Keeps threatening me. ME! Trumpy IRONTOE!!! Peh! That useless son of a goblin had better wise up and realize that in this burg if you ain't with [" .. eq.say_link("Who is Kane?",false,"Kane") .. "], you are against him. And if you are against him.. burp.. you ain't nothing but a corpse.");		
		elseif(e.message:findi("kane")) then
			e.self:Say("Ah.. burp.. that piece of cr.. eh? Oh its you.. Kane runs this burg.. or at least he will.. if [" .. eq.say_link("Who is Sragg?",false,"Sragg") .. "] can get his act together.");		
		...
		e.self:Say("You had better just be.. <BURP!>.. minding your own business if you know what is good for you. Gnollbreath. We Irontoes are proud and strong bunch of dwarves from [" .. eq.say_link("Where is Kaladim?",false,"Kaladim") .. "]. We crush weaklings like you. <BURP>");
	end
end
...
function event_trade(e)
	...
	if(moredrinks == 4) then
		e.self:Say("BWAH hah hah! I knew you wasn't the loser you.. <gulp>.. <gulp>.. appear to be! I will have to tell [" .. eq.say_link("Who is Kane?",false,"Kane") .. "].. <burp>.. all about you. Maybe get you in the gang!");
		e.other:Ding();
