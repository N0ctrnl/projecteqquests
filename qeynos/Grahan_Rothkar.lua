...
function event_say(e)
	if(e.message:findi("hail")) then
		e.self:Say("Hail, " .. e.other:GetName() .. "! What business do you have in the pens of the Qeynos Arena? Have you a report of the [" .. eq.say_link("What escaped catman?",false,"escaped catman") .. "]?");
	elseif(e.message:findi("escaped catman")) then
		e.self:Say("We had a catman imprisoned here for use in future gladiator battles. He escaped. In the process, he bit off my assistant's hand which held the key to pen number seven. I now seek to hire a warrior to [" .. eq.say_link("I will retrieve key seven",false,"retrieve key seven") .. "].");
	elseif(e.message:findi("retrieve key seven")) then
		e.self:Say("Hmmm. You seem a bit young, but I shall give you a chance. Seek out the catman named Nomala. He most likely returned to [" .. eq.say_link("What Kerra Isle?",false,"Kerra Isle") .. "]. He is not very powerful, but his people are surely watching over him. Retrieve the key for me and be rewarded.");
...
function event_trade(e)
	...
	if(item_lib.check_turn_in(e.trade, {item1 = 20031})) then
		e.self:Say("I thank you. I must admit I had my doubts, but you have proven yourself a true warrior. I salute you. You can be of some assistance to me.It see ms as though there has been a [" .. eq.say_link("What second escaped gladiator?",false,"second escaped gladiator") .. "] and I have a reward waiting for a human warrior.");
