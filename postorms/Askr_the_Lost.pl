sub EVENT_SAY {
  if (defined $qglobals{pop_poj_valor_storms}) {
    ...
    if (defined $qglobals{pop_pos_askr_the_lost} && $qglobals{pop_pos_askr_the_lost} == 1) {
      #checks for giant head handin
      if ($text =~ /It was me/i) {
      ...
      if ($text =~ /continue/i) {
        ...
        quest::setglobal("pop_pos_askr_the_lost", 2, 5, "F");
        #used to differentiate between the two [continue] statements
      }
    }
...
sub EVENT_ITEM {
  if (defined $qglobals{pop_poj_valor_storms} && $qglobals{pop_poj_valor_storms} == 1) {
    if (!defined $qglobals{pop_pos_askr_the_lost}
        && (   plugin::check_handin(\%itemcount, 11486 => 1)
            || plugin::check_handin(\%itemcount, 28749 => 1)
            || plugin::check_handin(\%itemcount, 28781 => 1)
            || plugin::check_handin(\%itemcount, 28782 => 1)))
    {
      #check for any of the 4 storm giant heads in the database
    ...
    if (defined $qglobals{pop_pos_askr_the_lost} && $qglobals{pop_pos_askr_the_lost} == 3 && plugin::check_handin(\%itemcount, 11488 => 1)) {
      ...
      #flagged for Bastion of Thunder
    ...
    if (defined $qglobals{pop_pos_askr_the_lost} && $qglobals{pop_pos_askr_the_lost} == 2 && plugin::check_handin(\%itemcount, 11487 => 1)) {
      ...
      #handed in sealed back of verity
    }
  } else {
