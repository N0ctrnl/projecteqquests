...
sub EVENT_SAY {
  if (defined $qglobals{shaman_pre} && $qglobals{shaman_pre} == 1) {
    if ($text=~/hail/i) {
      quest::say("Ah so the Elder Spirit of Enlightenment has sent you to me. I do have a task for you if you wish to help the cause of the spirits across the lands. I have been charged with making an elixer that will aid the spirits for a time. Are you [" . quest::saylink("I am interested",0,"interested") . "] in aiding us?");
    }
  }
  if (defined $qglobals{shaman_pre} && $qglobals{shaman_pre} == 1) {
...
sub EVENT_ITEM {
  if (defined $qglobals{shaman_pre} && $qglobals{shaman_pre} == 1 && plugin::check_handin(\%itemcount, 1678 =>1 )) {
    quest::say("You can now be called a heyokah, but there is much you may still do. I do require several [" . quest::saylink("What ingredients?",0,"ingredients") . "] for the potion I am required to make and should you assist me, there is a spirit with even greater tasks that will give you audience.");
