...
sub EVENT_SAY { 
	...
	if ($text=~/promise/i){
		quest::say("Since I was a boy, Xanuusus has entrusted me with a pair of holy symbols, one of Karana, and one of Tunare. I guarded them faithfully since that day until recently. I had been patrolling Innothule swamp in hopes of finding the wayward trolls that had been disturbing those of Tunare's children that inhabit the wetlands. Unknown to me at the time, a new [" . quest::saylink("What presence?",0,"presence") . "] was there in the swamp that day.");
	}
	if ($text=~/presence/i){
		quest::say("I had just found a troll threatening a water moccasin a fair distance from the gates of Grobb. The troll looked to be dressed a bit differently then most of the other trolls I'd found recently. He had a patch over one eye, and his armor appeared to be of much higher quality then anything the Bashers normally carried. I paid it no heed though, and continued planning my [" . quest::saylink("What attack?",0,"attack") . "] on the unsuspecting troll.");
	}
	if ($text=~/attack/i){
		quest::say("Just as I was beginning to weave my first spell against the troll, a large wet figure struck me by surprise knocking me to the ground. I staggered to my feet and looked at my attacker. It was a larger then normal troll holding a rather large curved sword and shield. It scowled at me with a sort of enraged hunger as it licked its lips in apparent anticipation of a fresh [" . quest::saylink("What meal?",0,"meal") . "].");
	}
	if ($text=~/meal/i){
		quest::say("As fate would have it the troll would go hungry that night. Although I was badly injured from his initial strike, I was able to stand and weave the roots of the nearby trees around his feet which allowed me time to gate away before more of his friends arrived. It wasn't until I arrived in the plains of Karana that I realized that I had lost my holy [" . quest::saylink("What emblems?",0,"emblems") . "].");
	}
	if ($text=~/emblems/i){
