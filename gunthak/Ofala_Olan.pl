...
sub EVENT_SAY { 
	if ($text=~/Hail/i){
		if ($class eq "Enchanter"){
			quest::say("Salutations! It is nice to see one such as you in these parts; I do get lonely for some intelligent conversation around here once in awhile. The majority of the time however, I am much too preoccupied [" . quest::saylink("What are you researching?",0,"researching") . "]. There are so many interesting things to discover in a new area!");
		}
		else{
