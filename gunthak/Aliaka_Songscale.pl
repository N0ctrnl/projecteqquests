...
sub EVENT_SAY { 
	...
	if ($text=~/fragments of the dark ice/i){
		quest::emote("smirks, 'Forgive my hesitance in trusting a stranger, but the frozen shards are quite painful, and removing them would be even more so. Perhaps we can trade services? I will give you some willingly if you would complete a [" . quest::saylink("What task?",0,"task") . "] first.'");
	}
	if ($text=~/task/i){
