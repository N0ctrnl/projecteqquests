sub EVENT_SAY { 
  if($text=~/hail/i){
    quest::say("Have you come for training young one?  The way of the Beastlord teaches many things about the forces of nature.  The [" . quest::saylink("What forces?",0,"forces") . "] in the thicket are very interesting.");
  }
  if($text=~/forces/i){
    quest::say("Yes, the force that compels a creature to adjust to their surroundings in particular.  The Saruek have seem to adapted to their environment very well.  I am studying the effects of their claws.   It seems the different [" . quest::saylink("What variety?",0,"variety") . "] of Saruek have special enhancements.");
  }
  if($text=~/variety/i){
    quest::say("Well, for example the Firefall Saruek can attack with claws that burn the flesh, the Deathmaw attack with poisonous claws. the shredder Saruek are especially skilled in fending off the Tegi that threaten them, and lastly there is a more uncommon Claw Beast that is very strong but does not appear that often.  I believe I could take their claws and affix them to a weapon to [" . quest::saylink("How enhance it?",0,"enhance") . "] it.");
  }
  if($text=~/enhance/i){
