...
sub EVENT_SAY {
  if ($text=~/hail/i) {
    quest::say("'Hello to you as well. I am Elder Patriarch Ixthal and I continue training when the Prime Patriarch becomes overwhelmed with duties. I will train you if you seek to one day [" . quest::saylink("become a Patriarch of Cabilis") . "].");
  }
  if ($text=~/become a patriarch of cabilis/i) {
    quest::say("I thought you might. I can sense your spirit is strong. For your first mission, you will need to use the spirits to guide you to collect some [" . quest::saylink("rare items") . "] we need on a daily basis in order to keep up with the demands of the city. This mission will also be great [" . quest::saylink("training") . "] for you.");
  }
  ...
  if ($text=~/continue training/i) {
    quest::say("That's the spirit. For your next task we will require you to show your skills at locating a [" . quest::saylink("missing member") . "] of the Patriarchs of Cabilis.");
  }
...
sub EVENT_ITEM {
  if (plugin::check_handin(\%itemcount, 12844 => 1, 12845 => 1, 12633 => 1, 48045 => 1)) {
   quest::say("You have returned unharmed with the items I asked for. Here is your reward now. If you wish to [" . quest::saylink("continue training") . "] please let me know.");
   quest::summonitem(48046); # Item: Claw of the Scout
   quest::exp(1000);
   quest::ding();
  }
  elsif (plugin::check_handin(\%itemcount, 48051 => 1, 48046 => 1)) {
    quest::say("Because of you, we now know that Welgaz is safe and will be returning to us shortly. Take this to represent our appreciation for your deeds completed here today. Please return soon once you have decided to [" . quest::saylink("aid the cause") . "].");
