...
sub EVENT_SAY {
 if($text=~/hail/i){ #Test of Betrayal (Greenmist Quest 5/8)
  ...
  quest::emote("spies the khukri sheath at your side. 'This cannot be. I was told a duke would attend the matter. Still, if the Lords have sent you, then you must be able to complete the task. The [" . quest::saylink("traitor") . "] most certainly must be stopped from delivering the [" . quest::saylink("Charasis Tome") . "]. The [" . quest::saylink("recipients") . "] appear quite formidable and I cannot handle such a deed, not single-handed.'");
  }
 elsif($text=~/traitor/i){ #Test of Betrayal (Greenmist Quest 5/8)
  quest::say("I am afraid I do not know. The babble is that he is from Cabilis, but he could have been a disguised sarnak. Who knows? The tink is that at night within the Outland territory, near the old skinless outpost, they are to meet. We should depart at once. Go get outfitted and let me know when you are [" . quest::saylink("ready to depart") . "].");
  }
 elsif($text=~/charasis tome/i){ #Test of Betrayal (Greenmist Quest 5/8)
  quest::say("The Charasis Tome, the tome of the old city of Charasis. It was taken from this temple and must be returned. The one sent to apprehend the traitor who stole the tome must be [" . quest::saylink("ready to depart") . "] with me at once.");
  }
 elsif($text=~/recipients/i){ #Test of Betrayal (Greenmist Quest 5/8)
  quest::emote("shows signs of fear and his face turns olive. 'I do not know. The [" . quest::saylink("spider riders") . "]!! I have only heard tales of them from dying lizards. Evil skinless stubs. From the wounds inflicted on the lizards, I can tell they are vicious opponents. Bloated and frayed flesh, disembowelment. Their steeds must have also joined the fray, for some of the victims were covered in webs.'");
  }
