...
sub EVENT_SAY {
 if($text=~/hail/i){
  quest::say("You dare to intrude upon the presence of a Lord of Pain?!  Such foolish acts can only be those of a knight in training.  Are you a [" . quest::saylink("knight in training") . "]?");
  }
 elsif($text=~/knight in training/i){ #Test of the Pawn (Greenmist Quest 1/8)
  quest::say("If you are a pawn, then let us begin with a simple test.  Complete the test and you shall be rewarded with a new khukri.  Do you [" . quest::saylink("wish to perform the test of a pawn") . "]?");
  }
...
sub EVENT_ITEM {
 if(plugin::check_handin(\%itemcount, 12381 => 1, 5120 => 1)) { #Test of the Pawn (Greenmist Quest 1/8)
  quest::say("You have learned to behead your opponents with great precision. Take the khukri of the knave. It shall prove most formidable in combat when wielded by a young crusader. You may now assist us with a [" . quest::saylink("greater concern") . "] than your training.");
