...
function event_say(e)
	...
	if(e.message:findi("hail")) then
		e.self:Say("Greetings, and may the pain of the ancients guide you. You have come to us for guidance, have you not? We are the Hierophants of Cabilis and we guide the young Scale Mystics. All petitioners shall speak with me of [" .. eq.say_link("temple tasks") .. "].");
	...
	elseif(e.message:findi("iron cudgel")) then
		if (e.other:GetFaction(e.self) <= 4) then
			e.self:Emote("shakes his head and waves a finger in your face. If you are looking to be handed the Iron Cudgel of the Clairvoyant then you are sadly mistaken. Perhaps if you were to gather a few [" .. eq.say_link("lost skulls") .. "] for the temple we may find you worthy to wield one.'");
		else
...
function event_trade(e)
	...
	elseif(e.other:GetFaction(e.self) < 5 and e.other:Class() == "Shaman" and item_lib.check_turn_in(e.trade, {item1 = 12403},0)) then --Full Component Mortar
		e.self:Say("We appreciate your service. Take a few copper for your deed as well as some of our cursed waters. They will provide you with nourishment. As for future tasks, we are searching for a few [" .. eq.say_link("lost skulls") .. "] and i am sure you are searching for your [" .. eq.say_link("iron cudgel of the clairvoyant") .. "] And i also hear that the furscales are in need of some broodlings to do some manual labor. Tell them Oxyn sent you.");
	...
	elseif(e.other:GetFaction(e.self) <= 4 and item_lib.check_turn_in(e.trade, {item1 = 12724, item2 = 12725, item3 = 5141})) then
		e.self:Say("We are in your debt," .. e.other:GetCleanName() .. " . You are truly one who shall collect all the lost ancient skulls. Take your weapon. Go to Hierophant Zand and he shall guide you further. Tell him you are [" .. eq.say_link("the chosen saviour") .. "].");
		e.other:Faction(445, 10); 				--Scaled Mystics
		e.other:Faction(441, 5); 				--Legion of Cabilis
