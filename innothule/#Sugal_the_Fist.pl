...
sub EVENT_SAY {
    ...
    if ($race eq "Froglok") {
      if ($client->HasSkill(0) && $client->HasSkill(1) && $client->HasSkill(2) && $client->HasSkill(3) && $client->HasSkill(36)) {
        if (defined $qglobals{token} && $qglobals{token} == 1) {
          ...
          quest::say("You're late! Feast of flies, what a weakling you are! We have no time to lose. [" . quest::saylink("No rest") . "] for the strong.");
        }
      } else {
  ...
  if ($text=~/no rest/i) {
    if ($race eq "Froglok") {
      if ($client->HasSkill(0) && $client->HasSkill(1) && $client->HasSkill(2) && $client->HasSkill(3) && $client->HasSkill(36)) {
        quest::say("You've slept your last easy night, recruit. If you want to stay a common footman, then keep hopping. But if you think you can be elite, if you think you can hack being a commando, then you've come to the [" . quest::saylink("right place") . "]. Welcome to the Guja.");
      } else {
        quest::say("Wander away, tad. You do not have what it takes.");
      }
    } else {
  ...
  if ($text=~/truly ready/i) {
    if ($client->GetSkill(0) >= 75 && $client->GetSkill(1) >= 75 && $client->GetSkill(2) >= 75 && $client->GetSkill(3) >= 75 && $client->GetSkill(36) >= 75) {
      quest::say("Good. That's a foundation we can build on. Now for your first round of training. A Guja must be adept at surviving in all forms of terrain. The swamp is warm, moist and comfortable. But you must learn to [" . quest::saylink("live anywhere") . "].");
    } else {
  ...
  if ($text=~/third notch/i) {
    if (defined $qglobals{token} && $qglobals{token} == 2) {
      quest::say("Though the trolls represent a threat to our people, their threat is distant. The Lizardmen of The Feerrott, however, present a real and menacing danger. I know that they already feed on our young and plan our demise. But we will [" . quest::saylink("act first") . "].");
    }
  }
  if ($text=~/act first/i) {
    if (defined $qglobals{token} && $qglobals{token} == 2) {
  ...
  if ($text=~/another option/i) {
    if ($race eq "Froglok" && defined $qglobals{token} && $qglobals{token} == 3) {
      quest::say("The lizardmen have challenged us, demanding that you represent Gukta. Should you win, war will be averted and many lives spared. Of course, it may mean your [" . quest::saylink("own life") . "].");
    }
  }
  if ($text=~/own life/i) {
...
sub EVENT_ITEM {
  ...
  if (plugin::check_handin(\%itemcount, 63098 => 1)) { # Notched Gukta Token
    if ($race eq "Froglok" && defined $qglobals{token} && $qglobals{token} == 1) {
      if ($client->GetSkill(0) >= 165 && $client->GetSkill(1) >= 165 && $client->GetSkill(2) >= 165 && $client->GetSkill(3) >= 165 && $client->GetSkill(36) >= 165) {
        quest::say("Hmm, You've learned much. You know how to survive in a wide variety of environments. You've learned the ways of watery warfare. Yes, I will notch your token. But the [" . quest::saylink("third notch") . "] is something you must earn on your own.");
        quest::exp(10000);
  ...
  if (plugin::check_handin(\%itemcount, 63103 => 1, 63104 => 1)) { # Twice Notched Guja Token, Lizardman Samples
    if ($race eq "Froglok" && defined $qglobals{token} && $qglobals{token} == 2) {
      quest::say("I wish I could say I welcome your return. But news has reached us. Apparently you did your job too well. The Lizardmen are even now readying for war. We cannot afford that fight. The council has decided we must . . .accept [" . quest::saylink("What option?",0,"another option") . "].");
      quest::exp(10000);
