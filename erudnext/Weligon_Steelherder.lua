...
function event_say(e)
	...
	elseif(e.message:findi("deepwater task")) then   
		...
		elseif(e.other:GetFaction(e.self) < 4) then
			e.self:Say("We here at the Deepwater Temple must tend to the [" .. eq.say_link("Peacekeeper battlestaff") .. "] and the [" .. eq.say_link("Deepwater harpoon") .. "] as well as other duties such as [" .. eq.say_link("ocean protection") .. "]. There is always something we must do.");  
		else
