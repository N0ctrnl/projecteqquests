...
function event_say(e)
	...
	elseif(e.message:findi("yes")) then
		e.self:Say("That is good, " .. e.other:GetName() .. ". Our followers are few but our faith is strong. With the efforts of those few, our destiny may be reached in time. We must walk with perseverence and devotion, much like the tides that, over time, can destroy a mighty stone cliff. Have you [" .. eq.say_link("come to serve our Lord") .. "], " .. e.other:GetName() .. "?");
...
function event_trade(e)
	...
	if(item_lib.check_turn_in(e.trade, {item1 = 1761})) then
		e.self:Say("Wonderful work, friend " .. e.other:GetName() .. ". One less land dwelling, flea ridden, primitive for our wondrous lord to sweep aside when the apocalypse is upon us. Here is your reward. Perhaps if you gather enough molars, we can offer you a [" .. eq.say_link("quest of greater importance") .. "].");
