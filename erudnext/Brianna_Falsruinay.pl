...
sub EVENT_SAY {
 if($text =~ /Hail, Brianna Falsruinay/i) {
	quest::say("Hello Traveler, I am afraid I don't recognize you to be a local around these parts although I could have been buried in my work. I have been quite busy since [" . quest::saylink("Elisi") . "] turned up missing.");
 }
 if($text =~ /Elisi/i) {
	quest::say("Elisi Nasin is a very well known enchanter and courier from Erudin. It is part of her trade to travel to Qeynos and share knowledge with the residents of the Order of Three. She often travels once or twice a month to deliver and pick up her most recent workings. However, she still has not returned from her most recent [" . quest::saylink("travel") . "]. We fear the worst for her; pirates have been rampant around these parts lately. We should send [" . quest::saylink("someone to Qeynos") . "] soon but have not had the time to do so.");
 }
...
sub EVENT_ITEM {
  if (plugin::check_handin(\%itemcount, 21699 => 1)) {
    quest::say("Jalorin gave this to you? This is quite odd because this is not Elisi's handwriting and there is clearly something wrong. I really do think for some reason that Jalorin is involved so if you are [" . quest::saylink("still willing to help") . "] I would like you to help me find out if he is being truthful. I must warn you though, if Jalorin is in fact involved with her disappearance this could get dangerous.");
  }
