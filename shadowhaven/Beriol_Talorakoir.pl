...
sub EVENT_SAY {
  if ($text=~/hail/i) {
    quest::say("Hail, $name. And good day to you. What brings you to my quarters today? Do you wish to learn more about the [" . quest::saylink("What magic?",0,"magic") . "] that we study in our hallowed section of the Shadowhaven?");
  }
  if ($text=~/magic/i) {
    quest::say("It's always a pleasure to meet someone that shows interest in the magic we practice here. Our magic studies consist mostly of teleportation and the arts of elemental mastery. I'm Actually conducting a study now, but I' missing a few [" . quest::saylink("What books?",0,"books") . "].");
  }
  if ($text=~/books/i) {
    quest::say("Well, I have an old student that is now serving as an advisor to one of the most respected residents of Katta Castelleum. He has some specific books that i need to further my research, but i cannot take the time to travel so far away from my home to retrieve them. I seek out one who will [" . quest::saylink("I will bring your research",0,"bring my research") . "] to me from Dersino.");
  }
