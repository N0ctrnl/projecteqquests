...
sub EVENT_SAY {
  ...
  if($text=~/recipe/i){
    ...
    quest::say("Well ya see lad I had an ancient recipe passed down to me that told one how to create the holiest of ale. Its far gone now, however I recently had a vision of the ingredients and where they might be found. Do you wish to seek out the [" . quest::saylink("What components?",0,"components") . "] for me?");
  }
  if($text=~/components/i){
    # Disciple Symbol of Brell Serilis
    quest::say("Excellent lad! Let meh see here if I can remembur correctly. I know that you need Preserved hops and a Battleworn cantine. I know that these were carried by my good friend Torklar back when I was still a young lad training like you. However one day he adventured to an undead estate with a friend of his that was a skilled brewer. I believe they went there in search of some ingredients they needed for a recipe however neither of them ever returned. But before you go any further I must tell ye that you will need a specific kind of [" . quest::saylink("What kind of cask?",0,"cask") . "] to combine these items in.");
  }
  if($text=~/cask/i){
    # Disciple Symbol of Brell Serilis
...
sub EVENT_ITEM {
  ...
  if(plugin::check_handin(\%itemcount, 13036 => 1, 13035 => 1, 13943 => 1, 2429 => 1)){ 
    # Disciple Symbol of Brell Serilis
    quest::say("Ah ha! So ye are Bronlors chosen aye? Well den these fine brews can only mean one thing! Yep its dat youre a drinker like meh! Arg, if I only had me [" . quest::saylink("Had what recipe?",0,"recipe") . "].");
