...
function event_say(e)
	if(e.message:findi("Hail")) then
		e.self:Say("Well met, " .. e.other:GetCleanName() .. "!  I am Vacto Molunel.  If you are seeking to outfit yourself with the finest weapons in all of Kaladim. I am the one to see.  I also posses a [" .. eq.say_link("What rare talent?",false,"rare talent") .. "] you might find useful.");
...
function event_trade(e)
	...
	if(item_lib.check_turn_in(e.trade, {item1 = 13849, item2 = 13849, gold = 4})) then -- helm
		e.self:Say("Excellent. Here is your helm. Wear it with pride! And be sure to occasionally wipe out the insulating mucus that tends to build up on its underside. It will make your hair fall out. One more thing, would you be interested in [" .. eq.say_link("scarab boots") .. "] to match your helm?");
