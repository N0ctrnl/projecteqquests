...
sub EVENT_SAY {
  if($text=~/hail/i){
    quest::say("Greetings, $name, I am the Oracle of K'Aron. Do you quest for [" . quest::saylink("What knowledge?",0,"knowledge") . "] yourself, or do you offer that which I [" . quest::saylink("What do you seek?",0,"seek") . "].");
  }
  if($text=~/knowledge/i){
    quest::say("I have lived long and have insight into many matters, yet I despise charity. To learn from me, you must first bring me that which I [" . quest::saylink("What do you seek?",0,"seek") . "].");
  }
  if($text=~/seek/i){
    quest::say("I seek knowledge, which is superior to all things. And no knowledge is greater than that which is contained within the [" . quest::saylink("What Tome of Ages?",0,"Tome of AGes") . "].");
  }
  if($text=~/tome of ages/i){
    quest::say("The Tome of Ages has been compiled over the millennia by the Scribes of [" . quest::saylink("Who are the Dal?",0,"Dal") . "]. It was divided into seven distinct [" . quest::saylink("What books?",0,"books") . "] by the scribes to keep the knowledge from one such as me. That said, with aid, I shall unite these books and the Tome of Ages shall be mine.");
  }
  if($text=~/dal/i){
