...
function event_say(e)
	if(e.message:findi("hail")) then
		e.self:Say("Hello stranger. Welcome to Galliway's. Please excuse me. I am very [" .. eq.say_link("Why are you tired?",false,"tired") .. "] right now.");
	elseif(e.message:findi("tired")) then
		e.self:Say("I have just arrived home after a trip to Halas where I was supposed to pick up a shipment of furs for my father, Sneed. However it all went [" .. eq.say_link("What went wrong?",false,"wrong") .. "].");
	elseif(e.message:findi("wrong")) then
		e.self:Say("On my way back to Qeynos from Halas, I was attacked by a large group of hideous trolls and captured. They took me to [" .. eq.say_link("What is Broken Skull Island?",false,"Broken Skull Island") .. "].' A look of fear overcomes Gibson. 'They put me to work in the mines of Nadox. However one day, an overseer approached me and made a [" .. eq.say_link("What deal?",false,"deal") .. "] with me.");
	...
	elseif(e.message:findi("deal")) then
		e.self:Say("The deal was that if he let me go, I would help supply their mining operations with goods from our shop. I of course agreed. However when the first [" .. eq.say_link("What contact?",false,"contact") .. "], for the pirates came for the shipment, I was not able to get all the supplies they wanted.");
	elseif(e.message:findi("contact")) then
		e.self:Say("The contact for the trolls was a horrible little gnome named Ralo the Black.' Tears well up in Gibson's eyes. 'He was furious when I told him I didn't have all the supplies that he wanted. He lunged towards me and bit my hand. In the struggle, he managed to take my [" .. eq.say_link("What ring?",false,"ring") .. "].");
	elseif(e.message:findi("ring")) then
		e.self:Say("It was nothing magical, but it has very sentimental ties attached to it. My father, Sneed gave it to me when I was old enough to help around the shop.' Gibson lowers his voice a bit. 'My father is not known for giving, so I have always treasured it. I do not know what I am going to do. My ring is missing and I am sure that horrid gnome will be back for another shipment soon. Perhaps you could find it in your heart to  [" .. eq.say_link("I will help you",false,"help") .. "]me? I am sure I could come up with some sort of compensation for such a dangerous task.");
