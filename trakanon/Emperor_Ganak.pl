...
sub EVENT_SAY {
  ...
  if($text=~/veeshan's peak/i){
    quest::say("You wish to know of Veeshans Peak?! You are either very brave or very foolish! There is nothing there for the uninvited but a fiery death. I have been away from this plane of existence for a long time yet still I feel the presence of the ancient wurms. I am sure they sense my presence as well, being as I hold the only [" . quest::saylink("What key to the chambers?",0,"key to the chambers") . "] of the Ring of Scale.");
  }
  if($text=~/key to the chambers/i){
    quest::say("I am not so quick to grant someone the key to their death! If you truly wish your doom to fall upon you so soon then bring me the three medallions of the fallen Iksar tribes, Kylong, Obulus, and Jarsath. The tribes were long ago crushed by the warriors of the Kunzar and their remains lie buried beneath ages of rubble. Finding the lost medallions of my people will not be enough to prove yourself worthy of entry into the fortress of the great wurms -- there is another task that you must complete before you return the [" . quest::saylink("What medallions?",0,"medallions") . "].");
  }
  ...
  if($text =~ /glowing orb/i) {
    quest::say("The glowing orb is activated by a special key, an idol crafted in the likeness of the dread Trakanon, Reaver of Sebilis. I have this idol and will give it to you, but only if you give me two [" . quest::saylink("What medallions?",0,"medallions") . "] representing the Iksar tribes of old.");
  }
  if($text =~ /Sebilis/i) {
    quest::say("An adventurer that wishes to enter the inner city of Sebilis you say?? There are only two ways in, either the [" . quest::saylink("What glowing orb?",0,"glowing orb") . "] in the heart of the ruins in the outer city, or via one of the tortured souls that wanders the jungles nearby.");
  }
}
