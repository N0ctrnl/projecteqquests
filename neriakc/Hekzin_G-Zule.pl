...
sub EVENT_SAY { 
  if($text=~/Hail/i){
    quest::say("How's it going. scrub? Correct me if I'm wrong. but you appear to be a rogue. If you're a [" . quest::saylink("I am a new rogue",0,"new rogue") . "]. it's my job to test your skill.  If you are not. then by speaking with me. I see you show interest in [" . quest::saylink("I am joining this house of rogues",0,"joining this house of rogues") . "].");
  }
  if($text=~/i am a new rogue/i){
    quest::say("We of the Ebon Mask must first be sure you are skilled with a blade. At the same time. we are attempting to create a new type of armor for our rogues. So it is the test of each young rogue to take this bag into Nektulos and fill it with three moss snake skins and three snake fangs. then combine them. We get the bag and you earn a little respect as well as the [" . quest::saylink("What Dark Shield of Ebon?",0,"Dark Shield of Ebon") . "].");
...
sub EVENT_ITEM { 
  if (plugin::check_handin(\%itemcount, 13889 =>1 )) {
    quest::say("Fine work for a scrub. Maybe one day you will be able to test this new snakescale armor out. For the moment you shall take the [" . quest::saylink("What Dark Shield of Ebon?",0,"Dark Shield of Ebon") . "] if you do not own one already.");
