...
sub EVENT_SAY { 
  if($text=~/Hail/i){
    quest::say("Greetings, $name!  We of the Dead are proud to have you among us.  Your lineage is well known.  Still, you must learn to claw your way to the upper echelon.  There are many [" . quest::saylink("What menial tasks?",0,"menial tasks") . "] to be performed before you can truly be inducted as a member.");
  }
  if($text=~/menial tasks/i){
    quest::say("You cannot avoid the toil of peons. We have all spent our time in the lower ranks. There are duties such as [" . quest::saylink("I will collect beetle eyes",0,"collecting beetle eyes") . "].");
  }
  if($text=~/collecting beetle eyes|collect beetle eyes/i){
...
sub EVENT_ITEM {
  if(plugin::check_handin(\%itemcount, 13389 => 1)){#Box of Beetle Eyes
    quest::say("Fantastic work, my child! We shall store these for further experiments. Take this as extra payment for a fine job. You have done so well I believe you can assist in obtaining two [" . quest::saylink("What other components?",0,"other components") . "].");
