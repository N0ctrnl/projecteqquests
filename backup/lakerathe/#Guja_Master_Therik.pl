...
sub EVENT_SAY {
  if ($text=~/amphibious abilities/i) {
    quest::say("That's why you're here, recruit! The lake is a great place to train swimming and aquatic combat. And those skills are core to the Guja way of life. We use land and sea to assault our foes. Whoops. . .I'm going on again. Let's get to your [" . quest::saylink("training") . "].");
  }
  if ($text=~/training/i) {
    quest::say("First off, you need to get your swimming as high as possible. Get it to 130 and we can start. The numbers? Oh, that's just Sugal's own little accounting. Tell me when your [" . quest::saylink("My swimming is ready",0,"swimming is ready") . "].");
  }
...
sub EVENT_ITEM {
  if (plugin::check_handin(\%itemcount, 63099 => 1)) { # Therik's Tooth
    quest::say("Ha! Oh my, it's been awhile. So Sugal is still kicking, eh? Sending me frog teeth. . .what a pain he is. Hmm . . .something wrong? Oh! You haven't fully developed your [" . quest::saylink("amphibious abilities") . "]. I'll get to the point.");
  }
  ...
  if (plugin::check_handin(\%itemcount, 63112 => 1, 63113 => 1, 63091 => 1)) { # First Marker, Second Marker, Guja Token
    ...
    quest::spawn2(51171, 0, 0, 1613, 156, -224, 310); # NPC: The_Lake_Lurker
  }
