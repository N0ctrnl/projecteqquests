...
sub EVENT_SAY {
  ...
  if ($text=~/prove my worth/i) {
    quest::say("So it shall be then that you will perform a task for the kin. As it happens, I was on a mission to speak with a [" . quest::saylink("What contact?",0,"contact") . "] of the shrine in the lands that do not sleep. This contact has vital information on the movements of the giants in that region and it is most important that we recover that information. Unfortunately, I have been called away on duty to further bolster our defenses and will not be able to meet with her.");
  }
  if ($text=~/contact/i) {
