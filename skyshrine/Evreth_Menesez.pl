...
sub EVENT_SAY {
  if ($faction == 1) {
    if($text=~/hail/i) {
      quest::say("Greetings, ally of the Claws of Veeshan! Allow me to introduce myself. I am Evreth Menesez of the Draconic Loyalist Fellowship, scholar and researcher of draconic arcane knowledge and practitioner of magery. I am currently researching the whereabouts of an [" . quest::saylink("What ancient dragon tome?",0,"ancient dragon tome") . "] that may contain lost arcane knowledge that could prove useful to the Claws of Veeshan, particularly the dragons and dragon-kin of Skyshrine.");
    }
    if($text=~/ancient dragon tome/i) {
      quest::say("The dragon sorcerer, Xervolithax, according to dragon lore, had penned a tome of arcane knowledge that he guarded with his life. It is told that Xervolithax fell to the Kromzek centuries ago in the Great Divide in a great battle. The only surviving apprentice of Xervolithax that survived the fateful incident recovered the tome from the fallen dragon sorcerer and divided it into [" . quest::saylink("What sections?",0,"sections") . "], which he then hid for fear of the tome falling into the hands of the Kromzek magic-users.");
    }
    if($text=~/sections/i) {
