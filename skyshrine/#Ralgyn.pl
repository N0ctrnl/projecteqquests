...
sub EVENT_SAY {
  if ($faction==1) {
    ...
    if ($text=~/what.*patrol/i) {
      quest::say("Ah yes, the patrol. Well in the Great Divide there is a collection of caves that the dragonkind holds as a sacred territory. For this reason we always have a number of our strongest up and coming younger ones train there with the great [" . quest::saylink("Who is Kardakor?",0,"Kardakor") . "]. They patrol the caved labyrinth to make sure it is free from unwanted intruders who are quickly dealt with upon discovery.");
    }
    if ($text=~/kardakor/i) {
