...
sub EVENT_SAY {
  if ($text=~/hail/i) {
    quest::say("It is most unfortunate that you do not have scales to defend your fragile mortal shell, $race, I can only hope that the armor we in the Shrine can provide you with will extend your life another fraction of a century. In addition to that you also might want a [" . quest::saylink("What special helm?",0,"special helm") . "] to cover up that scaleless face of yours. I understand that the Mother could not grace you with all our blessings, but at least we can try to make you look half-way decent.");
  }
