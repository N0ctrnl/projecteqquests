...
function event_say(e)
	if(e.message:findi("Hail")) then
		e.self:Emote("shows the signs of a great mystic. You can feel the power resonating from his aura. 'Greetings and may the cursed blessings of Cazic-Thule be yours. What may I do for you this fine evening? Perhaps I can [" .. eq.say_link("cure disease",false,"cure disease") .. "] or [" .. eq.say_link("heal",false,"heal") .. "] you, perhaps even [" .. eq.say_link("purge toxins",false,"purge toxins") .. "] from your system?");
	...
	elseif(e.message:findi("galdon vok nir?") and e.other:GetFaction(e.self) < 8) then
		e.self:Say("He is a merchant hiding in The Overthere. He is greedy and will not give the skull up easily. Seek him out and ask him what he would [" .. eq.say_link("What trade?",false,"trade") .. "] for the skull.");
	end
end
...
function event_trade(e)
	...
	if(item_lib.check_turn_in(e.trade, {item1 = 12736, item2 = 5143})) then
		e.self:Say("I see that you have found your answers. Now I must ask you to to retrieve for the council the skulls of the Di Nozok.  The sarnak in Lake of Ill Omen hold one of the skulls, while [" .. eq.say_link("Who is Galdon Vok Nir?",false,"Galdon Vok Nir") .. "] holds the other. Once you have retrieved them, give them to Hierphant Zand.");
