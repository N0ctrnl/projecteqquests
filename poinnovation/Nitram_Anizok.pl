...
sub EVENT_SAY {
  if($walking == 0) {
    if($text=~/Hail/i) {
      quest::say("Oh my hello! It has been such a long time since I have had visitors. Have you come to learn of [" . quest::saylink("advanced tinkering") . "] as well?");
    } elsif($text=~/advanced tinkering/i) {
      quest::say("Aye, I advanced to this plane due to my work on tinkering back in Ak`Anon. A grand city it is, but my abilities were compromised with the materials I had to work with there. My body and soul has come to rest here, forever coming up with new ideas. You should be aware though that this plane is not how it was when I arrived. Much [" . quest::saylink("construction") . "] has taken place.");
