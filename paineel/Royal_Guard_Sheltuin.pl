...
sub EVENT_SAY {
  ...
  if($text=~/i wish an audience with the overlord/i){
    quest::say("The Overlord is not seeing anyone at this time. however. I may have work for you. You seem to have some experience with the kobold annoyance in the region. Are you [" . quest::saylink("I am interested",0,"interested") . "]. $name?");
  }
  if($text=~/interested/i){
    quest::say("Then I shall give you a task. I assume you are aware of the kobold lair nearby. Many of our adventurous knights and priests crusade to destroy those pests so seeing many Erudites frequent the area is normal. However. it is not normal to see someone carry large crates into the lair and return empty handed. This person's movements also show he is attempting to remain unseen.....the [" . quest::saylink("What fool?",0,"fool") . "].");
  }
  if($text=~/fool/i){
...
sub EVENT_ITEM{
  ...
  if(plugin::check_handin(\%itemcount, 1776 => 1,1777 => 1, 18174 => 1, 1778 =>1)){#crate of ore,couriers head,suppliers head,sealed letter
    quest::say("'Honestly $name, I thought you would not return. Such strength and intelligence in a Knight of Fear I have not seen for quite some time. Wear this in pride of the ancient Ridossan. Perhaps when you are ready, i will give you a more [" . quest::saylink("What important task?",0,"important task") . "]. This conspiracy must be stopped.'");
    quest::exp(137180);
