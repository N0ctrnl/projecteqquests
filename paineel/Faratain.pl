...
sub EVENT_SAY { 
  if($text=~/Hail/i){
    quest::say("Salutations $name. If you are here to make use of the forges then feel free to pursue your crafts within this chamber. If you are a new Fell Blade in need of a suit of armor and weapon then I will assist you in crafting [" . quest::saylink("What armaments?",0,"armaments") . "] fitting of a young Shadow Knight.");
  }
  if($text=~/armaments/i){
    quest::say("You will require this specially prepared Mail Assembly Kit in order to construct a suit of Fell Blade Armor. The materials required vary depending on the piece of armor you desire to craft. Once you have been outfitted in the Fell Blade Armor. return to me and I will present you with a somewhat important [" . quest::saylink("What task?",0,"task") . "]. Do you desire to craft a [" . quest::saylink("fell blade helm") . "], a [" . quest::saylink("fell blade bracer") . "], [" . quest::saylink("fell blade gauntlets") . "], [" . quest::saylink("fell blade boots") . "], [" . quest::saylink("fell blade vambraces") . "], [" . quest::saylink("fell blade greaves") . "], or a [" . quest::saylink("fell blade breastplate") . "]?");
    quest::summonitem(17124); # Item: Mail Assembly Kit
  }
  if($text=~/task/i){
    quest::say("The blacksmiths of [" . quest::saylink("Who is Clan Kolbok?",0,"Clan Kolbok") . "] craft weapons using an ancient Kobold tradition that has been long forgotten by most of the kobold clans. I believe there is something to be learned from their traditions should we discover their intricacies. Go into the Warrens and locate the chambers of the kobold blacksmiths. There you should be able to find some record of their traditional techniques to return to me here in Paineel where they can be translated and studied.");
  }
  if($text=~/greaves/i){
