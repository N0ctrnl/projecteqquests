sub EVENT_SAY { 
  if($text=~/Hail/i){
    quest::say("Hello. researcher!  There are many things to be found here.  Our selection grows daily.  Would you [" . quest::saylink("I would like a book",0,"care for any book") . "]?");
  }
  if($text=~/books/i){
