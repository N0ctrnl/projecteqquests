sub EVENT_SAY { 
  if($text=~/Hail/i){
    quest::say("Hello. $name.  I make the finest statuettes in all the land.  Each one depicts a power on Norrath.  I have some of the most realistic and lifelike statues.  I only take [" . quest::saylink("What special orders?",0,"special orders") . "] as they are rather time-consuming to make.");
  }
