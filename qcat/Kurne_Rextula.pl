...
sub EVENT_SAY {
  if ($text=~/hail/i) {
    quest::say("Welcome to the new order of life, the life of service to our diseased Lord, Bertoxxulous. Did you encounter any of my [" . quest::saylink("What pets?",0,"pets") . "] on the way here? I hope not.");
  } elsif ($text=~/pets/i) {
    quest::say("My pets are the skeleton guardians of these catacombs. Some have been behaving erratically since the arrival of [" . quest::saylink("Who is Drosco?",0,"Drosco") . "].");
  } elsif ($text=~/drosco/i) {
    quest::say("Drosco used to be a loyal member of our shrine, until we learned the truth. The truth was that he was a mole from the Knights of Thunder. It appears they are assisting the Temple of Life in their pursuit of our eradication. Drosco was apprehended and boiled alive. I turned his boiled body into a zombie to march these halls. Now he must be stopped. He has begun to [" . quest::saylink("How is he compromising the location?",0,"compromise the shrine's location") . "].");
    ...
    if ($faction <= 4) {
      quest::say("I have been working with a man named Lord Grimrot. We are attempting to raise an army of undead in the Plains of Karana. When the time is right, we shall attack from within as well as from beyond the walls of Qeynos. This army needs one more commander. I must find a Bloodsaber to [" . quest::saylink("I will retrieve the commander",0,"retrieve the commander") . "]");
    } else {
...
sub EVENT_ITEM {
  ...
  if (plugin::takeItems(18805 => 1)) {
    quest::say("Good work! You shall rise quickly in our ranks of evil. Let no man stand in your way and never betray the shrine or you to will join our collection of undead. You can also assist me with a [" . quest::saylink("What new task?",0,"new task") . "].");
