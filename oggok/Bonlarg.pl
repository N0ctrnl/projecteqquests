...
sub EVENT_SAY {
  ...
  if($text=~/Hail/i){
    quest::say("Shadowknight master me be!!  $name prove to me that $name be worthy to be one with Greenblood and me give $name black shadow tunic.  You [" . quest::saylink("I want the black shadow tunic",0,"want black shadow tunic") . "]?");
  } elsif($sf < 201) {
