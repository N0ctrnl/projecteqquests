...
sub EVENT_SAY { 
  if($text=~/Hail/i){
    quest::say("Hello, $name. Don't you think this would be a wonderful place to perform a [" . quest::saylink("symphony") . "]?");
  }
  if($text=~/symphony/i){
    quest::say("It has been my dream for years to perform a symphony that would be remembered for ages to come. I have run into a few [" . quest::saylink("minor snags") . "], however.");
  }
  if($text=~/minor snags/i){
    quest::say("Several years ago I bought sheets of the most beautiful sounding music from a traveling merchant. He did not know the author of the work as it was simply signed 'the Maestro'. The melody I played from them was wondrous and invoked feelings in me I have never felt before. However, as I reached the middle of the music, I discovered it was [" . quest::saylink("missing two pages") . "]!");
  }
  if($text=~/missing two pages/i){
    quest::say("The pages that were missing were pages 24 and 25. I have been searching the world for a spot to hold my symphony and I have finally found it. Without the music and a new [" . quest::saylink("lute") . "] crafted by the famous instrument maker Forpar Fizfla, I do not think I can pull it off.");
  }
  if($text=~/lute/i){
    quest::say("Forpar Fizfla is an eccentric instrument maker from Ak'Anon. He makes enchanted instruments that enhance the sound of the [" . quest::saylink("music") . "]. If I am to perform the concert of all concerts, I must have one of his instruments. I went to Ak'Anon, seeking to purchase an instrument from him but he wouldn't even speak to me.");
  }
  if($text=~/music/i){
    quest::say("It makes me sad to know that the pages of this music may be spread across the world or destroyed. If you find and bring me the two missing sheets as well as a new lute, I would be happy to [" . quest::saylink("repay") . "] you in some way.");
  }
  if($text=~/repay/i){
