  ...
  if(quest::istaskactivityactive(216,10)){
    if($text=~/Hail/i) {
      if(defined $qglobals{halloween_race_victory} && !defined $qglobals{halloween_race_complete}) {
        ...
        quest::say("You managed to find your way through Nektulos to Neriak, but you did not as quickly as you could have... Let me know if you would like to [" . quest::saylink("I would like to try again",0,"try") . "] again.");
        quest::failtask(216);
      }
    }
    ...
    if($text=~/Hail/i) {
      quest::emote("nods solemnly at you, 'Have you also come to stand [" . quest::saylink("I have come to stand vigil",0,"vigil") . "] on this day of grim history?'");
    }
    if($text=~/Vigil/i) {
      quest::emote("looks at your closely, 'Ah, I see you are not familiar with the vigil for Ritrim Shives, my ancestor who cheated the creatures that dwell below the forest roots and lived to tell the [" . quest::saylink("What tale?",0,"tale") . "].'");
    }
    if($text=~/Tale/i) {
      quest::emote("smiles darkly, 'The Tale is a horrifying one... in exchange for the life of his only child, these creatures agreed to bestow upon Ritrim a dark and terrible [" . quest::saylink("What power?",0,"power") . "].'");
    }
    if($text=~/Power/i) {
      quest::emote("nods, 'The power to harm any living creature with a mere touch of the hand.. After a gruesome ceremony, when he was granted the dark power, he acted quickly and used his new power to slay the creatures even as they prepared to sacrifice his child, and feast upon his flesh... in his haste to subvert the agreement, Ritrim failed to realized the [" . quest::saylink("What toll?",0,"toll") . "] that the power would take upon him.'");
    }
    if($text=~/Toll/i) {
      quest::emote("chuckles, 'Using his new power drained his energy, leaving him weak and vulnerable. With his son, he moved towards his mount, a powerful black warhorse, a single enemy blocked his path... Ritrim reached out again with his power, killing his foe, but not quickly enough... a shout was given, and he felt a searing pain as his [" . quest::saylink("eyes") . "] were burned away.'");
    }
    if($text=~/Eyes/i) {
      quest::say("Blinded, and in great pain, Ritrim mounted his steed to make his escape, the voice of his son guiding him as he charged through the forest of Nektulos weaving a [" . quest::saylink("crooked path") . "] in order to evade his pursuers, and return to Neriak.");
    }
    if($text=~/Crooked/i) {
      quest::emote("smiles, 'If you are really that interested, I will summon you a dark [" . quest::saylink("What mount?",0,"mount") . "] so that you may follow the path for yourself.'");
    }
    if($text=~/Mount/i || $text=~/Try/i) {
      quest::emote("hands you a bridle, 'When you are [" . quest::saylink("I am ready",0,"ready") . "], you will need to ride with haste and reach the gates of Neriak before the fourth minute has passed.'");
