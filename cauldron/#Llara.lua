...
function event_say(e)
	...
	if(qglobals["Llara"] ~= nil) then
		...
		elseif(e.message:findi("amstaf")) then
			e.self:Say("I've known Amstaf since he was a child. We grew up in the same city together. Ever since I've known him there's always been something [" .. eq.say_link("unusual") .. "] about him.");
		elseif(e.message:findi("unusual")) then
			e.self:Say("When we were growing up we used to venture outside the city gates. Some times we would encounter a wandering ghoul or an undead spirit of some sort. He was always able to quickly dismiss the undead being with very little effort. I [" .. eq.say_link("learned") .. "] many things growing up with Amstaf.");
		elseif(e.message:findi("learn")) then
			e.self:Say("Amstaf told me many things when we were younger. Once Amstaf told me that life isn't about how you get there, it's about where you end up. I miss my dear friend. Its been a long time since I've seen him. The night of the snowstorm I dreamt about him. I remember seeing Amstaf's face. I saw something I've never [" .. eq.say_link("seen") .. "] before.");
		elseif(e.message:findi("see")) then
			e.self:Say("I saw desperation. I spoke to many people who knew Amstaf trying to find out if anyone heard anything about that night. No one knew anything about it. When I had reached the dwarven hunting grounds I ran into a woman named Walnan. She kept rambling on and on about the power of the Orb and something about Magi`kot. I didn't understand what she was saying. She didn't know anything about my situation either, but she did tell me that there were others that may be able to help me. She told me that I should find [" .. eq.say_link("Ruathey") .. "].");
