...
function event_say(e)
	if(e.message:findi("hail")) then
		e.self:Say(string.format("Pleasure to meet you %s. I am Lady Salinsa Deifdosan of House Marr. I have spent many hours on the battlefield in service of our glorious god however these days I spend my time training new [" .. eq.say_link("recruits") .. "].",e.other:GetName()));
	elseif(e.message:findi("recruits")) then
		e.self:Say("Well you see sense I am such a well seasoned adventurer my knowledge is very helpful to the young members of our house. That is why I have developed a system of hunting and gathering assignments that reward the new recruit with a set of armor that they work very hard to craft themselves. Are you a [" .. eq.say_link("I am a young cleric of House Marr",false,"young cleric of House Marr") .. "]? If so I will be happy to offer my assignments to you.");
	elseif(e.message:findi("young cleric of house marr")) then
		e.self:Say("Great! Let me first begin by explaining how you will be creating your own armor. You will use this magical kit I have presented you with to gather numerous [" .. eq.say_link("recipe components") .. "]. Your magical kit will be used to gather specific materials that will form together in certain numbers and quantities to create an armor material. You must then take the material you have fashioned to a forge along with armor molds that I will supply you with to create your armor.");
		e.other:SummonItem(17259); --Gem Encrusted Mail Kit
	elseif(e.message:findi("recipe components")) then
		e.self:Say("There are many different items that you will need to collect for your material components in all areas surrounding Freeport and our temple. I will be able to supply you with the mold for any armor piece you want to craft. I have the molds and recipes necessary for [" .. eq.say_link("Helms") .. "], [" .. eq.say_link("Bracers") .. "], [" .. eq.say_link("Armguards") .. "], [" .. eq.say_link("Boots") .. "], [" .. eq.say_link("Greaves") .. "], [" .. eq.say_link("Gauntlets") .. "] and [" .. eq.say_link("Breastplates") .. "]. When you are ready to craft a piece simply tell me what piece you would like to craft. For example if you wanted to create boots you would say, 'I want to craft boots'.");
