...
function event_say(e)
	if(e.message:findi("hail")) then
		e.self:Say("Greetings to you " .. e.other:GetName() .. ". I am Sten Harnak Virtuoso of Freeport. I have lived in Freeport and studied the art of songweaving here for all of my days. I am always eager to see new faces here in our concert hall. If you are here for a performance I am afraid it does not start till a bit later but if you are a young bard in training then I might have some [" .. eq.say_link("What work",false,"work") .. "] for you.");
	elseif(e.message:findi("what work")) then
		e.self:Say("Well I have numerous concerts here that I must prepare for so I always must be sure that I have the best possible support as far as my fellow bards go. I am always seeking new young talent to sing in our concerts. If you are a bard I have made tasks for you to complete to see if you are a worthy songweaver. These tasks will test your determination and battle skills. I would only allow one that has completed all of my [" .. eq.say_link("specific tests") .. "] to sing in my concert hall.");
	elseif(e.message:findi("specific test")) then
		e.self:Say("If you are ready to begin your training I will assign you a number of tests. These tests will enable you to outfit yourself with a suit of armor only worn by the most respected musicians of Freeport. First. I will present you with a Songweavers Assembly Kit. In this kit you will combine certain [" .. eq.say_link("what recipes",false,"recipes") .. "] that will be magically formed into an armor material. Each one of these materials will need to be combined in a forge with the appropriate mold to make your armor piece.");
		e.other:SummonItem(17257); -- Item: Songweavers Assembly Kit
	elseif(e.message:findi("what recipe")) then
		e.self:Say("I have the material recipes available for Songweavers Platemail [" .. eq.say_link("Helms") .. "], [" .. eq.say_link("Bracers") .. "], [" .. eq.say_link("Armguards") .. "], [" .. eq.say_link("Boots") .. "], [" .. eq.say_link("Greaves") .. "], [" .. eq.say_link("Gauntlets") .. "], and [" .. eq.say_link("Breastplates") .. "]. When you are ready to craft a specific piece simply tell me what armor piece you want to craft. For example if you would wish to craft a helm tell me [I want to craft a helm]. Also please keep in mind that the components necessary for crafting your breastplate material are the most difficult to collect so you might want to collect them last.");
