...
sub EVENT_SAY { 
  if($text=~/Hail/i){
    ...
    quest::say("Those damned [" . quest::saylink("What trolls?",0,"trolls") . "]! They stole everything and destroyed my ship! The [" . quest::saylink("What House?",0,"House") . "] will have my head over this!");
    quest::emote("coughs violently.");
  }
  if ($text=~/house/i) {
    quest::say("I work for a trading house run by a group of nobles in Neriak. They are not known for being too forgiving when their [" . quest::saylink("What valuables?",0,"valuables") . "] are lost.");
  }
  if ($text=~/valuables/i) {
    quest::say("We were hauling many things. The one thing in particular that I am worried about was a trunk full of tomes. I do not know what they were about exactly, but my masters explicitly told me not to let them leave my sight.' Yiln coughs. 'I have got to find those tomes!' Yiln tries to move but yells out in excruciating pain. I cannot get up. You must go to Paineel and find [" . quest::saylink("Who is Jerith?",0,"Jerith") . "]. Tell him I sent you and tell him about Osaftars. You will be rewarded well for your time. If you recover the books bring them back to me.");
  }
  if ($text=~/jerith/i) {
