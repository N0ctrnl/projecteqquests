...
sub EVENT_SAY { 
  if ($text=~/hail/i) {
    if ( $client->GetGlobal("Fatestealer") ==1) { #Rogue 1.5
      ...
      quest::say("How is life treating you, bud? What are you doing around the mines? Either you are a [" . quest::saylink("I am a member of 628",0,"member of 628") . "] or you are lost. If you are lost, I can't help you. I ain't no guide.");		 
    } else {
    ...
    if ($faction <= 4) {
      quest::say("It's my duty to assign [" . quest::saylink("What guild tasks?",0,"guild tasks") . "] to all new members of Mining Guild 628.");
    } elsif ($faction <= 5) {
    ...
    if ($faction <= 4) { # Requires high amiable on live
      quest::say("So you want to earn a parrying pick? Consider it an honor that I am even speaking of this with you. The guild had these picks made just for us. It is a magic item used to fend off attacks. Before you can have one, you will have to do me a [" . quest::saylink("What great favor?",0,"great favor") . "].");
    } elsif ($faction <= 5) {
      quest::say("Don't take this personally, but I can't quite trust you with such matters. Maybe a few less Butcherblock bandits would prove your worth.");
    } else {
      quest::say("The word around the mines is that you are not to be trusted. You'd best leave before my dagger finds a new home in your back.");
...
sub EVENT_ITEM {
  if (plugin::check_handin(\%itemcount, 13931 => 4)) { # RunnyEye Warbeads
    quest::say("Good work. We shall add these to the stash. Here is your reward, as promised. Be happy with it and continue your work. Maybe soon you shall be able to [" . quest::saylink("What parrying pick?",0,"earn a parrying pick") . "].");
    ...
    quest::say("Excellent work!! You are quite an asset to this mining guild. Please accept this Parrying Pick 628 for your great service. If you truly wish to serve our guild. Go and speak with Mater. Tell him you are [" . quest::saylink("I am ready to earn Mining Pick 628",0,"ready to earn Mining Pick 628") . "].");
    quest::faction(322, 10); # Miners Guild 628
    quest::faction(223, -10); # Circle Of Unseen Hands
    quest::faction(379, -10); # Butcherblock Bandits
    quest::faction(241, 10);  # Deeppockets
