...
sub EVENT_ENTER {
  if(plugin::check_hasitem($client, 36000)) { 
    $client->Message(15,"A stern, commanding looking dwarf looks your way. He has an unsettling gleam in his eye. 'Oy there! been waitin' for ya! Welcome to Kaladim! Tha name's Dravel. If you be wishin' ta join us, hand me your note and we will begin your trainin' Oh, remember to ask me about that [" . quest::saylink("What tome?",0,"tome") . "] there in yer inventory. Wouldn't want ta ferget about that!'");
  }
}
...
sub EVENT_SAY {
  if($text =~ /hail/i ) {
    if(defined $qglobals{berserk_epic} && $qglobals{berserk_epic} >= 1) {
      ...
      quest::say("Greetin's $name! You say ye wish to join up with us? That be great! Read the note in yer inventory and then hand it to me and we can start yer trainin! I almost forgot. Ask me about that [" . quest::saylink("What tome?",0,"tome") . "] in yer inventory if ye want to know how to use it.");
    }
  }	
  if($text =~/tome/i) {
