...
sub EVENT_SAY  {
  if ($text=~/hail/i) {
    if ($faction <= 7) {
      quest::say("What business do you have here?!! This here is the mine and that means if you ain't a [" . quest::saylink("I am a member of Miner's Guild 628",0,"member of Miner's Guild 628") . "], you'd best be moving on!!");
    } else {
    ...
    if ($faction <= 4) {
      quest::say("Well, then, get off yer rump and give us a hand! If you don't have a mining pick, then get down there and use your fingernails!! If you're new and you want to earn a pick, you can [" . quest::saylink("I will volunteer to exterminate the rats",0,"volunteer to exterminate the rats") . "] that have been infesting the mines!!");
    } elsif ($faction <= 5) {
    ...
    if ($faction <= 4) {
      quest::say("Well, as you most likely know, we are a non-profit organization, at least here in Kaladim. We need to make our profit elsewhere and that means a little bit of thieving. Presently we need loyal members to [" . quest::saylink("I will collect the gnome profit",0,"collect the gnome profit") . "].");
    } elsif ($faction <= 5) {
      quest::say("Don't take this personally, but I can't quite trust you with such matters. Maybe a few less Butcherblock bandits would prove your worth.");
    } else {
      quest::say("The word around the mines is that you are not to be trusted. You'd best leave before my dagger finds a new home in your back.");
...
sub EVENT_ITEM {
  if (plugin::check_handin(\%itemcount, 13054 => 4)) { # Giant Rat Pelt
    quest::say("It's about time you managed to smash these four!! Here. You do good work. We could use someone like you to [" . quest::saylink("I will run to the gnome city",0,"run to the gnome city") . "]."); 
