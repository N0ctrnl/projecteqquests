...
function event_say(e)
	if(e.message:findi("hail")) then
		e.self:Say("Eh!!? What do I see before me? A young upstart? Some child pretending to be a great mind? If you wish to test your mettle then you will assist Tergon. Will you [" .. eq.say_link("Help Tergon") .. "] or are you far [" .. eq.say_link("too superior") .. "] to deal with such things?");
	...
	elseif(e.message:findi("further tasks")) then
		e.self:Say("Not all experience is gained upon the battlefield. We magicians must heighten our minds to become formidable opponents. I see much promise in you, " .. e.other:GetName() .. ". I will require you to [" .. eq.say_link("travel abroad") .. "] toward Freeport.");
	elseif(e.message:findi("travel abroad")) then
		e.self:Say("You will go to Freeport and seek out the Academy of Arcane Science. There you shall find my brother Retlon. He has scribed some new spells in my [" .. eq.say_link("personal spellbook") .. "]. Hand him this note as proof of your alliance. He works closely with Master Dooly Jonkers.");
		e.other:SummonItem(1717); -- Item: Sealed Letter
		e.other:Ding();
	elseif(e.message:findi("defector")) then
		e.self:Say("It seems a gnome magician by the name of Toko Binlittle has gone and left the guild. He joined forces with the [" .. eq.say_link("Pirates of Gunthak") .. "]. Find him. He must be destroyed. Our secrets cannot be known. Return his head to me and I shall give you the [" .. eq.say_link("Elemental Grimoire") .. "].");
...
function event_trade(e)
	...
	if(item_lib.check_turn_in(e.trade, {item1 = 13333})) then
		e.self:Say("So, you have survived. There is no doubt in my mind that you achieved this solely with your own powers. Only a dim one requires the assistance of others. Take this. May it help you in your pursuit of greatness. [" .. eq.say_link("Further tasks") .. "] may bring you to that point.");
	...
	elseif(item_lib.check_turn_in(e.trade, {item1 = 13387})) then
		e.self:Say("Thank you " .. e.other:GetName() .. ". I had no doubt you would fulfill my will and return my spellbook.. here is your reward for such effort. Now I have news of a larger matter. It has to do with a [" .. eq.say_link("deflector") .. "].");
		e.other:Faction(245,3,0); -- Faction: Eldritch Collective
		e.other:Faction(238,-3,0); -- Faction: Dark Reflection
		e.other:Faction(239,-3,0); -- Faction: The Dead
		e.other:Faction(255,3,0); -- Faction: Gem Choppers
