...
sub EVENT_SAY {
  if (quest::istaskactivityactive(17,3)) {
    ...
    if ($text=~/hail/i) {
      ...
      quest::say("I'm glad you managed to escape the slave warrens in one piece. There are many [" . quest::saylink("What others?",0,"others") . "] who were not as lucky. Here, take this armor we found on a kobold. It's not much, but it should keep you safe");
    ...
    if ($text=~/others/i) {
      ...
      quest::say("These few here. They were injured when the revolt broke out. I'm trying to help, but we have a short supply of [" . quest::saylink("What bandages?",0,"bandages") . "], and more wounded keep coming. I fear many will not survive the hour.");
