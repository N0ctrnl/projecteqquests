...
sub EVENT_SAY { 
	if($text=~/Hail/i){
		quest::say("Greetings stranger. It's nice to have visitors of the living sort. Seems like my work here is never done. When I'm not having to dig fresh graves and keep the mausoleum clean I'm having to refill old graves that have been dug up by the [" . quest::saylink("What restless dead?",0,"restless dead") . "] or them disrespectful [" . quest::saylink("What grave robbers?",0,"grave robbers") . "]!");
	}
	if($text=~/restless dead/i){
		quest::say("That's right. We've got our fair share of undead lurking around our fine city. Most of the sorry souls aren't as troublesome as the vampyres and we've even got a few [" . quest::saylink("What friendly spirits?",0,"friendly spirits") . "] right here in this very mausoleum.");
	}
	if($text=~/friendly spirits/i){
		quest::say("Every so often when Norrath is near its zenith a group of spirits gathers at the podium and chairs over there. Seems they were some of the original Loyalists that arrived here on Luclin and helped build our great city of Katta Castellum. Their bodies are at rest in the stone crypts along these mausoleum walls and guarded by the [" . quest::saylink("What gargoyle warders?",0,"gargoyle warders") . "]. Yet even in death their spirits convene to discuss the condition of Katta Castellum and the teachings of Tsaph Katta.");
	}
	if($text=~/gargoyle warders/i){
		quest::say("The gargoyles that guard the residents of this mausoleum are quite fascinating. They were carved late in the construction of Katta Castellum by a very talented dwarven stone worker. Gundle Chislebeard. Gundle used a type of stone found in the very cliffs Katta Castellum is built upon. At first the gargoyles were nothing more than impressive statues but then over time they began to [" . quest::saylink("How did they change?",0,"change") . "].");
	}
	if($text=~/change/i){
