...
function event_timer(e)
	if(timer == jillin) then
		e.self:Say("[" .. eq.say_link("Who is Jillin?",false,"Jillin") .. "]? Jillin? Where did he go? Hrumph!");
	end
end
...
function event_say(e)
	...
	elseif(e.message:findi("jillin")) then
		e.self:Say("Jillin is my courier. He was supposed to take this pot of [" .. eq.say_link("What stew?",false,"stew") .. "] over to Deputy Lowmot in Guardian Stronghold. It is just about ready and Mayor Gubbin hates cold stew!"); 
...
local item_lib = require("items");
	if (item_lib.check_turn_in(e.trade,  {item1 = 13958})) then
		e.self:Say("Well it is about time!  The mayor gets very upset if he does not have the freshest of carrots in his stew.  Here is the money for the carrots.  Be off with you.  Now. where the heck did [" .. eq.say_link("Who is Jillin?",false,"Jillin") .. "] go?");
	...
	elseif (item_lib.check_turn_in(e.trade,  {item1 = 13957})) then
		e.self:Say("Oh excellent! These carrots are perfect! The finest Reebo has ever sent us. The mayor will be so pleased. Here is the payment for the carrots. Excuse me, but I must finish preparing the stew. Hmm. Where the heck did [" .. eq.say_link("Who is Jillin?",false,"Jillin") .. "] go?");
