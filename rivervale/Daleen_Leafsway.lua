...
function event_say(e) 
	...
	elseif(e.other:GetFaction(e.self) < 5 and e.message:findi("starving")) then
		e.self:Say("Deputy Eigon! I forgot! I was supposed to bring him some turnips to eat while he is on patrol! Oh... He asked so nicely, too. I feel bad that I forgot. If only someone would take these [" .. eq.say_link("What turnips?",false,"turnips") .. "] to the Deputy..");
...
function event_timer(e)
	if (timer == blurt) then
		e.self:Say("Oh Dear.. I can't believe I forgot.. He must be [" .. eq.say_link("Why is he starving?",false,"starving") .. "]!");
	end
end
