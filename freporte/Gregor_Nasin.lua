...
function event_say(e)
	if(e.message:findi("hail")) then
		e.self:Say("Sit down and name your poison!  I know we are lacking in originality. but I intend to create some [" .. eq.say_link("exotic drinks") .. "].");
	elseif(e.message:findi("exotic drinks")) then
		e.self:Say("Interested in exotic drinks. are we?  I am attempting to create some of these drinks I read about in this book I found. It was called the [" .. eq.say_link("Barkeep's Compendium") .. "].  I will first need to get my hands on a few [" .. eq.say_link("exotic ingredients") .. "].");
	elseif(e.message:findi("barkeep's compendium")) then
		e.self:Say("I found the book amongst some garbage littering the ground in Freeport East.  It appears to have been created by an ogre named [" .. eq.say_link("Clurg") .. "].  I am sure he wants it returned. but I will never part with it.  That is... unless I get a few [" .. eq.say_link("exotic ingredients") .. "] I need to experiment with.");
	elseif(e.message:findi("exotic ingredients")) then
		e.self:Say("I desire a [" .. eq.say_link("kiola nut") .. "], [" .. eq.say_link("Erud's Tonic") .. "], [" .. eq.say_link("honey jum") .. "] and a [" .. eq.say_link("Roalindl fish") .. "].  If I could get those. I would no longer need to hold on to the [" .. eq.say_link("Barkeep's Compendium") .. "].  Whoever returned the ingredients to me could have it!!");
...
function event_trade(e)
	...
	if(item_lib.check_turn_in(e.trade, {item1 = 13118,item2 = 13383,item3 = 13952,item4 = 13340})) then
		e.self:Say("Now I have every ingredient mentioned in the Barkeep Compendium. Here. You take it. <..click!.> Whoops!! I just closed it. It's magically sealed, I never closed it before. It's useless to you. I have no need for it any longer. Maybe you can return it to [" .. eq.say_link("Clurg") .. "] for some type of reward.");
