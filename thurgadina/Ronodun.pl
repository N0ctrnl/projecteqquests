...
sub EVENT_WAYPOINT_ARRIVE {
  ...
  if($wp==23) {
    ...
    quest::signalwith(115141,1,8000); # NPC: Petcas_Coldbeard
  }
  if($wp==45) {
    $npc->SetAppearance(1);
  }
...
sub EVENT_SIGNAL {
  if($signal==1) {
    ...
    quest::signalwith(115141,2,8000); # NPC: Petcas_Coldbeard
  }
  if($signal==2) {
    ...
    quest::signalwith(115141,3,4000); # NPC: Petcas_Coldbeard
  }
  if($signal==3) {
    quest::emote("hiccups!");
    quest::signalwith(115141,4,3000); # NPC: Petcas_Coldbeard
  }
  if($signal==4) {
    quest::signalwith(115141,5,8000); # NPC: Petcas_Coldbeard
  }	
  if($signal==5) {
    quest::say("Thank ye, Pet. Yer most kind.");
    quest::signalwith(115141,6,7000); # NPC: Petcas_Coldbeard
  }
}	
