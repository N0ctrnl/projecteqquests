...
sub EVENT_SAY {
  if ($faction <= 3) {
    if ($text=~/hail/i) {
      quest::say("I wasn't always just a merchant, ye know, I used to work at the temple and spent most of my time in the study of ancient texts and arts. I actually learned a lot about the dark arts in those days. Ye know... necromancy. Oh, I'd never touch the stuff, mind ye, but I'll bet I could make a fine set of plate fer shadowknights if'n I had the [" . quest::saylink("What are the right materials?",0,"right materials") . "].");
    }
    if ($text=~/right materials/i) {
      quest::say("Glad to see ye're interested, $name. The materials would vary by the piece of armor o' course. I could probably forge a fine set of [" . quest::saylink("bracers",0,"bracers") . "], [" . quest::saylink("gauntlets",0,"gauntlets") . "], [" . quest::saylink("greaves",0,"greaves") . "], [" . quest::saylink("boots",0,"boots") . "], [" . quest::saylink("armplates",0,"armplates") . "], a [" . quest::saylink("helm",0,"helm") . "], or a [" . quest::saylink("breastplate",0,"breastplate") . "].");
    }
    if ($text=~/helm/i) {
