...
sub EVENT_SAY {
  if ($faction <= 3) {
    if ($text=~/hail/i) {
      quest::say("Huh? Don't you know better than to interrupt a Coldain enjoying his ale? I may be a user of [" . quest::saylink("What priestly magic?",0,"priestly magic") . "] but that doesnt mean I can't bash your head in.");
    }
    if ($text=~/priestly magic/i) {
      quest::say("Aye. An [" . quest::saylink("What old one?",0,"old one") . "] not much taken with idle chit chat.");
    }
    if ($text=~/old one/i) {
      quest::say("Very old, very wise, and on and on. You must have some point to bothering me. I'm not a merchant and I don't contract myself out for work. I've performed works for the Dain. There isn't much I [" . quest::saylink("What is it you haven't done?",0,"haven't done") . "]...");
    }
