...
sub EVENT_SAY {
  if($text=~/hail/i){
    quest::say("Hail, $name. I am the leader of this beleagured troop of rangers. Even though the once fair Kithicor slowly succumbs to the [" . quest::saylink("What darkness?",0,"darkness") . "] at hand, we shall not abandon the woods without a fight.");
  } elsif($text=~/darkness/i){
    quest::say("The denizens of these woods are slowly being overrun by dark fiends and shadowed men. As valiantly as we have fought, our numbers dwindle. We know very little about our aggressors, but if we [" . quest::saylink("I will acquire some information",0,"acquire some information") . "] or perhaps [" . quest::saylink("What fine weapons?",0,"fine weapons") . "], then we may yet have a chance to retake the woods once more.");
