...
sub EVENT_SAY {
  if(quest::istaskactivityactive(215,4)){
    ...
    if(!defined $qglobals{halloween_monster_mash}){
      if($text=~/Hail/i) {
        quest::say("Lurgh miss [" . quest::saylink("What happened to your friend?",0,"friend") . "].");
      }
      if($text=~/Friend/i) {
        quest::say("Lurgh had friend that battle hard. We fight lots. Skeleton friend [" . quest::saylink("dance") . "] for me, but he gone now.");
      }
      if($text=~/Dance/i) {
        quest::say("Lurgh laugh when skeleton dance. But now sad,  [" . quest::saylink("What skeleton?",0,"skeleton") . "]ate by bear and no more dancin' for Lurgh.");
      }
      if($text=~/Skeleton/i) {
        quest::say("Lurgh see lots of skeleton here. Maybe one be friend? You [" . quest::saylink("I will help",0,"help") . "] Lurgh find friend?");
      }
      if($text=~/Help/i) {
