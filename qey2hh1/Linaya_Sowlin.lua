...
function event_say(e)
	if(e.message:findi("hail")) then
		e.self:Say("Greetings!  It is always nice to meet another traveler.  The roads of the Plains of Karana are heavily trodden, but sparsely patrolled.  Be careful of bandits and especially of giants.  I have seen a few since my [" .. eq.say_link("Why did you move from the Jaggedpine?",false,"move from the Jaggedpine") .. "].");
	elseif(e.message:findi("note")) then
		e.self:Say("The note spoke of the [" .. eq.say_link("Who are the Unkempt Druids?",false,"Unkempt Druids") .. "]. I did not get a chance to read it all, only a glimpse. Perhaps the man still has the note. Too bad. I am sure Gerael Woodone of the druids of Surefall Glade should look at it.");
...
function event_trade(e)
	...
	if(item_lib.check_turn_in(e.trade, {item1 = 13945})) then
		e.self:Say("Oh thank Tunare you showed up when you did. I was at a nearby merchant house when a fellow dropped a [" .. eq.say_link("What note?",false,"note") .. "] and I picked it up and read it. It talked of the [" .. eq.say_link("Who are the Unkempt Druids?",false,"Unkempt Druids") .. "] and before I could read on, the man swiped it from my hands. I ran for dear life, for surely he would kill me for reading the note. I think I lost him in the woods but I am not sure. Please stay with me a while to be sure.");
		e.other:Ding();
