...
function event_waypoint_arrive(e)
	if(e.wp == 1) then
		e.self:Say("I believe here is where I am to wait for the [" .. eq.say_link("What blessed oil?",false,"blessed oil") .. "].");
	elseif(e.wp == 2 or e.wp == 5) then
		e.self:Say("Cleanse your souls, sinners! The way of life is found through the [" .. eq.say_link("Who is the Prime Healer?",false,"Prime Healer") .. "]. Repent and join us!");
	end
end

function event_say(e)
	if(e.message:findi("hail")) then
		e.self:Say("Greetings, child of life. The way of the [" .. eq.say_link("Who is the Prime Healer",false,"Prime Healer") .. "] shall set you free and cleanse your soul. I hope to see you at [" .. eq.say_link("What mass?",false,"mass") .. "].");
	...
	elseif(e.message:findi("mass")) then
		e.self:Say("Mass is not being held out here as yet. I am waiting for the [" .. eq.say_link("What blessed oil?",false,"blessed oil") .. "] from the Temple of Life. You may attend services there.");
	elseif(e.message:findi("blessed oil")) then
		e.self:Say("The blessed oil is necessary during services. It is blessed by High Priestess Jahnda at the [" .. eq.say_link("What Temple of Life?",false,"Temple of Life") .. "]. If you are going in that direction, please stop at the temple and remind High Priestess Jahnda that brother Estle is waiting for his blessed oil.");
