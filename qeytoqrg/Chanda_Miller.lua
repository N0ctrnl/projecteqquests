...
function event_say(e)
	...
	if(fac < 6) then
		...
		elseif(e.message:findi("baobob sent me")) then
			e.self:Say(string.format("Oh.. Hello. My name is Chanda. %s is your name? That's a nice name. My [" .. eq.say_link("Who is your brother?",false,"brother") .. "] sent you to talk to me? You must want some bear.. um.. [" .. eq.say_link("What clothes?",false,"clothes") .. "].",e.other:GetName()));
		...
		elseif(e.message:findi("clothes")) then
			e.self:Say("I um.. know how to make.. umm.. [" .. eq.say_link("cloaks",false,"cloaks") .. "] and [" .. eq.say_link("boots",false,"boots") .. "]. Oh yes! and belts! I love to make bear skin belts! Um.. I really like [" .. eq.say_link("belts",false,"belts") .. "]. My [" .. eq.say_link("Who is your brother?",false,"brother") .. "] says I have to start charging more gold for my stuff though..");
		elseif(e.message:findi("cloak")) then
			e.self:Say("I.. um.. can make.. um.. a good cloak for you. You have to get me a bear skin of high quality and my brother says I should not make them unless you also give me 21 gold pieces. I used to give them away but [" .. eq.say_link("Who is Baobob?",false,"Baobob") .. "] got mad at me so I don't do that anymore.");
