...
function event_say(e)
	...
	elseif(e.message:findi("illusion")) then
		if(illusionResponse == 0) then
			e.self:Say("Yes!! I seem to have dropped some of my [" .. eq.say_link("What illusion cards?",false,"illusion") .. "] cards.");
			illusionResponse = 1;
		else
...
function event_trade(e)
	...
	local wowwie = e.self:Say("Wow!! I thought I lost this for good. All together I lost the [" .. eq.say_link("What illusion cards?",false,"illusion") .. "] cards: Ebon Lotus, Library of Erudin, Chrono Cyclone and Diamond Vale. Man!! Those are rare cards!");
...
function event_waypoint_arrive(e)
	...
	elseif(e.wp == 9) then
		...
		e.self:Say("Hey! Did I just drop something?");
	elseif(e.wp == 11) then
		e.self:Say("I lost some of my [" .. eq.say_link("What illusion playing cards?",false,"illusion") .. "] playing cards. And they were the rare ones too! Darn it! If anyone finds them, please return them to me.");
	end
end
