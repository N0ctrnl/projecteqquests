...
function event_say(e)
	...
	if(e.message:findi("hail")) then
		e.self:Say("Greetings, my friend. What brings you so far from the trodden path? I hope you do not plan on stealing my [" .. eq.say_link("fishing spot") .. "].");
	elseif(e.message:findi("fishing spot")) then
		e.self:Say("I need this spot!! I fish to feed my growling tummy. I can no longer [" .. eq.say_link("venture into Kaladim") .. "] and eat in its fine taverns.");
	elseif(e.message:findi("venture into kaladim")) then
		e.self:Say("I have been disgraced!! I used to be a respected paladin of the Church of Underfoot until I failed to complete a very [" .. eq.say_link("important task") .. "] for Lord Datur Nightseer. He stripped me of my Cape of Underfoot. I cannot walk the halls of Kaladim without my cape.");
	elseif(e.message:findi("important task")) then
		e.self:Say("I was ordered by Lord Nightseer to carry the [" .. eq.say_link("Chalice of Conquest") .. "] to a noble band of paladins in the mountains of Rathe. While there, the entire camp was overtaken by a band of orcs. During the battle there was a [" .. eq.say_link("strange occurrence") .. "].");
	...
	elseif(e.message:findi("strange occurrence")) then
		e.self:Say("A large fray raged. Brave, valiant paladins fought vile, green-skinned orcs and while this happened, I spied the [" .. eq.say_link("Chalice of Conquest") .. "] being taken away from the camp. I was then distracted by a well placed orcish blade. After I defeated the orcs in my way, I turned to see a blue orc running toward the horizon. I gave chase to no avail. I lost the chalice and all respect. If only a [" .. eq.say_link("noble Paladin of Underfoot") .. "] would assist me...");
	elseif(e.message:findi("candle of bravery")) then
		e.self:Say("The Candle of Bravery is ceremonial and is lit when one sips from the [" .. eq.say_link("Chalice of Conquest") .. "]. The candlestick was left with a fellow Paladin of Underfoot in the Rathe Mountains. The candle itself was lost and I do not know where Priestess Ghalea purchases them.");
	elseif(e.message:findi("noble paladin of underfoot")) then
		if(fac > 4) then
			...
			e.self:Say("Yes, you shall do, noble " .. e.other:GetName() .. ". Take this. This is the Chalice Case. It is intended for the Chalice of Conquest and the [" .. eq.say_link("Candle of Bravery") .. "]. If you should get both, be sure to place them within the Chalice Case and combine them. Take the full Chalice Case to Lord Nightseer in Kaladim and I shall be redeemed and you shall surely earn a fine paladin's reward. Let your quest begin in the mountains of Rathe.");
