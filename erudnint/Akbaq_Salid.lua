...
function event_say(e)
	if (e.other:GetFaction(e.self) <= 5) then --Require indifferent or greater faction    
		if(e.message:findi("hail")) then
			e.self:Say("Greetings, " .. e.other:GetName() .. ". I have recently returned to Erudin since hearing word of the titan spirits that inhabit the Stonebrunt Mountains. Should you be heading in that direction there are some [" .. eq.say_link("samples") .. "] that I am interested in acquiring."); 
		elseif(e.message:findi("sample")) then
			e.self:Say("The titan spirits of the Stonebrunt Mountains appear in several various animal forms seemingly with different areas of influence in the spirit world. I am interested in conducting some experiments on the hides of the giant leopard titan and the giant sabretooth titan. Once I have acquired those hides I will seek to conduct experiments on the [" .. eq.say_link("others") .. "]."); 
		elseif(e.message:findi("other")) then
			e.self:Say("Once I have finished my studies of the feline titans pelts I am interested in experimenting with the hides of the strongest of the stonebrunt titans. the gigantic gorilla and the gargantuan panda. The [" .. eq.say_link("ancient snake") .. "] that inhabits those mountains is another matter that you may be interested in as well."); 
