...
if ($text=~/what tome/i) {
 ...
 sub EVENT_ITEM {
  if (plugin::check_handin(\%itemcount, 55189 => 1, 55187 => 1, 55190 =>1, 55188 =>1)) {#Pages Handin
   ...
   quest::say("This is all [" . quest::saylink("wrong") . "]!");
    my $x = $npc->GetX();
    my $y = $npc->GetY();
    my $z = $npc->GetZ();
    my $h = $npc->GetHeading();
