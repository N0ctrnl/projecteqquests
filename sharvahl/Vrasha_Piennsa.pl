...
sub EVENT_SAY {
  if($text =~ /hail/i ) {
    if(defined $qglobals{berserk_epic} && $qglobals{berserk_epic} >= 1) {
      ...
      quest::say("If you are ready to join the Ireblood Ragers, read the note in your inventory and then hand it to me.  Make sure to ask me about that [" . quest::saylink("What tome?",0,"tome") . "] in your inventory as well.  It will help you greatly during your adventures.");
    }
  }
