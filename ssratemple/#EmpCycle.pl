...
sub EVENT_TIMER {
  if ($timer eq "EmpCycle") {
    ...
    # The Blood lockout has passed, spawn him - if he's not up.
    if (($qglobals{Emperor} == 1) && !$entity_list->GetNPCByNPCTypeID(162189) && !$entity_list->GetNPCByNPCTypeID(162064) && !defined $qglobals{BloodDead} && !$entity_list->GetNPCByNPCTypeID(162189) && ($EmpPrep == 0)) {
    ...
    elsif (!$entity_list->GetNPCByNPCTypeID(162189)){
      quest::spawn2(162189,0,0,877.0,-325.0,400.5,384); ##Blood_of_Ssraeshza
    }
  }
  if ($timer eq "EmpPrep") {
